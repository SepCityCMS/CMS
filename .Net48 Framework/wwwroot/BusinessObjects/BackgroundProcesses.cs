...
namespace wwwroot
{
    ...
    public class BackgroundProcesses
    {
        ...
        public void Import_Wholesale2b_URL(string url, long FeedID, string AccessKeys, bool AccessHide, bool ExcPortalSecurity, bool Sharing, string PortalIDs)
        {
            try
            {
                var engine = new DelimitedFileEngine(typeof(SepCommon.Models.Wholesale2b));
                ...
                {
                    var Category = SepCommon.DAL.Categories.Category_Get_By_Name(record.category_1, FeedID);
                    if (Category.CategoryName != record.category_1)
                        try
                        {
                            SepCommon.DAL.Categories.Category_Save(SepFunctions.GetIdentity(), "0", record.category_1, string.Empty, "|41|", AccessKeys, AccessHide, "|2|", false, "|2|", string.Empty, string.Empty, string.Empty, PortalIDs, string.Empty, string.Empty, Sharing, ExcPortalSecurity, 0, string.Empty, string.Empty, FeedID);
                        }
                        catch
                        {
                            // Do Nothing
                    ...
                    var Product = SepCommon.DAL.ShoppingMall.Product_Get_By_ImportID(record.Item, FeedID);
                    long sProductId = 0;
