...
// ***********************************************************************
// <copyright file="approval_chains_waiting.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    /// <summary>
    /// Class approval_chains_waiting.
    /// Implements the <see cref="System.Web.UI.Page" />
    /// </summary>
    /// <seealso cref="System.Web.UI.Page" />
    public partial class approval_chains_waiting : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Approval");
                    FilterDoAction.Items[2].Text = SepFunctions.LangText("Resend Emails");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Chain Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Module Name");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No waiting approvals have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    if (FilterDoAction.Value == "DeleteApproval")
                    {
                        var sDeleteResult = SepCommon.DAL.ApprovalChains.Waiting_Delete(sIDs);
                    ...
                    else
                    {
                        ...
                        if (arrIds != null)
                        {
                            for (var i = 0; i <= Information.UBound(arrIds); i++)
                                ...
                                {
                                    conn.Open();
                                    using (var cmd = new SqlCommand("SELECT ModuleID,UniqueID FROM ApprovalXML WHERE XMLID=@XMLID", conn))
                                    {
                                        cmd.Parameters.AddWithValue("@XMLID", arrIds[i]);
                                        using (SqlDataReader RS = cmd.ExecuteReader())
                                        {
                                            if (RS.HasRows)
                                            {
                                                RS.Read();
                                                SepFunctions.Approval_Chain_Check(SepFunctions.toInt(SepFunctions.openNull(RS["ModuleID"])), SepFunctions.openNull(RS["UniqueID"]), true);
                                            }
                        ...
                        DeleteResult.InnerHtml = "<div class=\"alert alert-success\" role=\"alert\">" + SepFunctions.LangText("Email(s) has been sent.") + "</div>";
                    }
                }
                catch
                {
                    if (FilterDoAction.Value == "DeleteApproval") DeleteResult.InnerHtml = "<div class=\"alert alert-danger\" role=\"alert\">" + SepFunctions.LangText("There has been an error deleting.") + "</div>";
                    else DeleteResult.InnerHtml = "<div class=\"alert alert-danger\" role=\"alert\">" + SepFunctions.LangText("There has been an error sending an email.") + "</div>";
                }
            ...
            {
                if (FilterDoAction.Value == "DeleteApproval") DeleteResult.InnerHtml = "<div class=\"alert alert-danger\" role=\"alert\">" + SepFunctions.LangText("You must select at lease one item to run an action.") + "</div>";
                else DeleteResult.InnerHtml = "<div class=\"alert alert-danger\" role=\"alert\">" + SepFunctions.LangText("You must select at lease one item to resend an email to.") + "</div>";
            }
        }
        ...
        {
            var dApprovalWaiting = SepCommon.DAL.ApprovalChains.GetApprovalWaiting(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dApprovalWaiting));
            return dv;
        }
