...
namespace SepCommon.Core
{
    ...
    public static partial class SepFunctions
    {
                    ...
                    using (var cmd = new SqlCommand(indexSQL, conn))
                    {
                        ...
                        if (RecordID > 0)
                        {
                            ...
                            using (SqlDataReader IndexRS = cmd.ExecuteReader())
                            {
                                ...
                                {
                                    IndexRS.Read();
                                    if (toLong(openNull(IndexRS["PortalID"])) > 0)
                                    {
                                        indexUrl = "/portals/" + toLong(openNull(IndexRS["PortalID"]));
                                    }
                                    else
                                    {
                                    ...
                                    switch (iModuleID)
                                    {
                                        case 10:
                                            ...
                                            {
                                                case ".docx":
                                                    sKeywords = DocX2Text(GetDirValue("downloads") + jDownloads.FileName);
                                                    break;

                                                case ".pdf":
                                                    sKeywords = PDF2Text(GetDirValue("downloads") + jDownloads.FileName);
                                                    break;
                                            }

                                            indexUrl += moduleUrl + openNull(IndexRS["RecordId"]);
                                            break;

                                        case 18:
                                            sKeywords = GetUserInformation("Sex", openNull(IndexRS["UserID"]));
                                            indexUrl += moduleUrl + openNull(IndexRS["RecordId"]) + "/" + Format_ISAPI(openNull(IndexRS["Title"])) + "/";
                                            break;

                                        default:
                                            sKeywords = openNull(IndexRS["Keywords"]);
                                            indexUrl += moduleUrl + openNull(IndexRS["RecordId"]) + "/" + Format_ISAPI(openNull(IndexRS["Title"])) + "/";
                                            break;
                                    }

                                    AddText(iModuleID, RecordID, openNull(IndexRS["Title"]), trimDescription ? Strings.Left(RemoveHTML(openNull(IndexRS["Description"])), 300) : openNull(IndexRS["Description"]), sKeywords, openNull(IndexRS["HTML"]), indexUrl, indexIsHtml, openNull(IndexRS["PostalCode"]), openNull(IndexRS["UserID"]));
                                }
                            }
                        }
                        else
                        {
                            using (SqlDataReader IndexRS = cmd.ExecuteReader())
                            {
                                while (IndexRS.Read())
                                {
                                    ...
                                    {
                                        case 10:
                                            var jDownloads = DAL.Downloads.Download_Get(toLong(openNull(IndexRS["RecordId"])));
                                    ...
                                    AddText(iModuleID, toLong(openNull(IndexRS["RecordId"])), openNull(IndexRS["Title"]), trimDescription ? Strings.Left(RemoveHTML(openNull(IndexRS["Description"])), 300) : openNull(IndexRS["Description"]), sKeywords, openNull(IndexRS["HTML"]), indexUrl, indexIsHtml, openNull(IndexRS["PostalCode"]), openNull(IndexRS["UserID"]));
                                }
                            }
                        }
                    }
        ...
        {
            if (ModuleActivated(5))
            {
                IndexRecord(5, 0);
            }

            if (ModuleActivated(9))
            {
                IndexRecord(9, 0);
            }

            if (ModuleActivated(10))
            {
                IndexRecord(10, 0);
            }

            if (ModuleActivated(12))
            {
                IndexRecord(12, 0);
            }

            if (ModuleActivated(18))
            {
                IndexRecord(18, 0);
            }

            if (ModuleActivated(19))
            {
                IndexRecord(19, 0);
            }

            if (ModuleActivated(20))
            {
                IndexRecord(20, 0);
            }

            if (ModuleActivated(31))
            {
                IndexRecord(31, 0);
            }

            if (ModuleActivated(35))
            {
                IndexRecord(35, 0);
            }

            if (ModuleActivated(37))
            {
                IndexRecord(37, 0);
            }

            if (ModuleActivated(41))
            {
                IndexRecord(41, 0);
            }

            if (ModuleActivated(44))
            {
                IndexRecord(44, 0);
            }

            if (ModuleActivated(61))
            {
                IndexRecord(61, 0);
            }

            if (ModuleActivated(63))
            {
                IndexRecord(63, 0);
            }
