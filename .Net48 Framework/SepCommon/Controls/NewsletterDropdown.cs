    ...
    public class NewsletterDropdown
    {
        ...
        public string Render()
        {
            ...
            using (SqlConnection conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                {
                    cmd.Parameters.AddWithValue("@PortalIDs", SepCommon.SepCore.Strings.ToString(SepFunctions.Get_Portal_ID()));
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        while (RS.Read())
                        {
                            output.AppendLine("<option value=\"" + SepFunctions.openNull(RS["LetterID"]) + "\"" + SepCommon.SepCore.Strings.ToString(Text == SepFunctions.openNull(RS["LetterID"]) ? " selected=\"selected\"" : string.Empty) + ">" + SepFunctions.openNull(RS["NewsletName"]) + "</option>");
                        }
                    }
                }
