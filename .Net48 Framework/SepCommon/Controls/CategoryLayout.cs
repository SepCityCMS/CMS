    ...
    public class CategoryLayout
    {
        ...
        public string Render()
        {
            ...
            using (SqlConnection conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                {
                    wc += " AND (";
                    string[] arrUserKeys = Strings.Split(UserAccessKeys, ",");
                    if (arrUserKeys != null)
                    {
                ...
                string CountSqlStr = "(SELECT Count(" + countID + ") AS RecordCount FROM " + countTable + " WHERE ";
                    ...
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        if (RS.HasRows)
                        {
                            ...
                            else
                            {
                                ...
                                intRecordCount = 0;
                                int iTotalRecords = RS.RecordsAffected;
                                while (RS.Read())
                                {
                                    long iCount = SepFunctions.toLong(SepFunctions.openNull(RS["Count1"])) + SepFunctions.toLong(SepFunctions.openNull(RS["Count2"])) + SepFunctions.toLong(SepFunctions.openNull(RS["Count3"])) + SepFunctions.toLong(SepFunctions.openNull(RS["Count4"]));
