...
// ***********************************************************************
// <copyright file="invoices.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class invoices : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Invoices");
                    FilterDoAction.Items[2].Text = SepFunctions.LangText("Mark as Paid");
                    FilterDoAction.Items[3].Text = SepFunctions.LangText("Mark as Not Paid");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Full Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("User Name");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Order Status");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Order Date");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No invoices have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            {
                if (FilterDoAction.Value == "DeleteInvoices")
                    try
                    {
                        var sDeleteResult = SepCommon.DAL.Invoices.Invoice_Delete(sIDs);
                ...
                if (FilterDoAction.Value == "MarkPaid")
                    try
                    {
                        var sDeleteResult = SepCommon.DAL.Invoices.Invoice_Mark_Paid(sIDs);
                        DeleteResult.InnerHtml = Strings.InStr(sDeleteResult, SepFunctions.LangText("Successfully")) > 0 ? "<div class=\"alert alert-success\" role=\"alert\">" + sDeleteResult + "</div>" : "<div class=\"alert alert-danger\" role=\"alert\">" + sDeleteResult + "</div>";
                    }
                    catch
                    {
                ...
                if (FilterDoAction.Value == "MarkNotPaid")
                    try
                    {
                        var sDeleteResult = SepCommon.DAL.Invoices.Invoice_Mark_Not_Paid(sIDs);
                        DeleteResult.InnerHtml = Strings.InStr(sDeleteResult, SepFunctions.LangText("Successfully")) > 0 ? "<div class=\"alert alert-success\" role=\"alert\">" + sDeleteResult + "</div>" : "<div class=\"alert alert-danger\" role=\"alert\">" + sDeleteResult + "</div>";
                    }
                    catch
                    {
        ...
        {
            var dInvoices = SepCommon.DAL.Invoices.GetInvoices(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value, SepCommon.SepCore.Request.Item("UserID"), RequireUserID: true);

            dv = new DataView(SepFunctions.ListToDataTable(dInvoices));
            return dv;
        }
