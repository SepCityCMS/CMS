...
namespace wwwroot
{
    ...
    public partial class advertising : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Site URL");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Target Zone");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Description");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Advertisement Preview");
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No advertisements have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.Advertisements.Advertisement_Delete(sIDs);
        ...
        {
            var dAdvertisements = SepCommon.DAL.Advertisements.GetAdvertisements(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dAdvertisements));
            return dv;
        }
