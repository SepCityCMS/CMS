...
namespace SepCityCMS.Server.DAL
{
    ...
    public static class Polls
    {
        ...
        public static string Poll_Delete(string PollIDs)
        {
            ...
            sReturn = SepFunctions.LangText("Poll(s) has been successfully deleted.");
        ...
        public static int Poll_Save(long PollID, string UserID, string Question, DateTime StartDate, DateTime EndDate, string PortalIDs, string OptionIDs, string OptionValues)
        {
            ...
            using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                arrOptionIDs = Strings.Split(OptionIDs, "||");
                arrOptionValues = Strings.Split(OptionValues, "||");
            ...
            intReturn = SepFunctions.Additional_Data_Save(isNewRecord, 25, Strings.ToString(PollID), UserID, SepFunctions.GetUserInformation("UserName", UserID), "Polls", "Polls", string.Empty);
                    ...
                    using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
                    {
                        ...
                        using (var cmd = new SqlCommand("SELECT PollOption FROM PNQOptions WHERE OptionID=@OptionID", conn))
                        {
                            ...
                            using (SqlDataReader RS = cmd.ExecuteReader())
                            {
                                ...
                                {
                                    RS.Read();
                                    sActDesc = SepFunctions.LangText("[[Username]] has voted for a poll") + Environment.NewLine;
            ...
            using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                using (var cmd = new SqlCommand("SELECT SUM(NumVotes) AS TotalVotes FROM PNQVotes WHERE PollID=@PollID", conn))
                {
                    ...
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        while (RS.Read())
                        {
                            Total = Total + SepFunctions.toLong(SepFunctions.openNull(RS["TotalVotes"]));
                        }
                    }
                }
