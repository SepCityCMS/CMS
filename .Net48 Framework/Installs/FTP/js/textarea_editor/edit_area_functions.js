...
EditArea.prototype.set_font = function(family, size) {
    ...
    var elems = [
        "textarea", "content_highlight", "cursor_pos", "end_bracket", "selection_field", "selection_field_text",
        "line_number"
    ];
...
EditArea.prototype.scroll_page = function(params) {
    ...
    if (dir == "up") {
        ...
        for (var i = this
            .last_selection["line_start"];
        i - diff > this.last_selection["line_start"] - nbLineToScroll;
        i--) {
            if (elem = _$("line_" + i)) {
        ...
        if (this.last_selection["selec_direction"] == "up") {
            for (line_nb = 0;
                line_nb < Math.min(this.last_selection["line_start"] - nbLineToScroll, lines.length);
                line_nb++) {
                new_pos += lines[line_nb].length + 1;
            }
            char_left = Math.min(lines[Math.min(lines.length - 1, line_nb)]
                .length,
        ...
        if (this.last_selection["selec_direction"] == "down") {
            ...
            view = "top";
            for (line_nb = 0;
                line_nb < Math.min(this.last_selection["line_start"] + nbLineToScroll - 1, lines.length, lines.length);
                line_nb++) {
                if (line_nb == this.last_selection["line_start"] - 1)
                    char_left = this.last_selection["selectionStart"] - new_pos;
                new_pos += lines[line_nb].length + 1;
...
EditArea.prototype.open_file = function(settings) {
    ...
    if (settings["id"] != "undefined") {
        ...
        if (!_$(this.files[id]["html_id"]) && id != "") {
            ...
            var close = "<img src=\"" +
                parent.editAreaLoader.baseURL +
                "images/close.gif\" title=\"" +
                this.get_translation("close_tab", "word") +
                "\" onclick=\"editArea.execCommand('close_file', editArea.filesIdAssoc['" +
                html_id +
                "']);return false;\" class=\"hidden\" onmouseover=\"this.className=''\" onmouseout=\"this.className='hidden'\" />";
            elem.innerHTML = "<a onclick=\"javascript:editArea.execCommand('switch_to_file', editArea.filesIdAssoc['" +
                html_id +
...
EditArea.prototype.display_file = function(id) {
    ...
    for (i in a_selects) {
        ...
        for (var j = 0; j < a_options.length; j++) {
