...
EditArea.prototype.check_line_selection = function(timer_checkup) {
    ...
    if (!this.smooth_selection && !this.do_highlight) {
        ...
        changes = this.checkTextEvolution(typeof(this.last_selection["full_text"]) == "undefined"
            ? ""
            : this.last_selection["full_text"],
            infos["full_text"]);
        ...
        if (this.last_selection["line_start"] != infos["line_start"] ||
            ...
            if (!this.settings["word_wrap"]) {
                this.selection_field.style.width = this.selection_field_text.style
                    .width = this.test_font_size.style.width = new_width + "px";
            }
            ...
            if (this.do_highlight == true) {
                ...
                content = content.substr(0, infos["curr_pos"] - 1) +
                    "\r\r" +
                ...
                content = "<span>" +
                    ...
                    .replace(/</g, "&lt;")
                    .replace(/>/g, "&gt;")
                    .replace("\r\r", "</span><strong>")
                    .replace("\r\r", "</strong><span>") +
                    "</span>";
                ...
                (infos["full_text"] != this.last_text_to_highlight &&
                (this.last_selection["line_start"] != infos["line_start"] ||
                    this.show_line_colors ||
                    this.settings["word_wrap"] ||
                    this.last_selection["line_nb"] != infos["line_nb"] ||
                    this.last_selection["nb_line"] != infos["nb_line"]))) {
        ...
        if (infos["line_start"] != this.last_selection["line_start"] ||
            ...
            if (infos["line_nb"] == 1 && (this.assocBracket[selec_char] || this.revertAssocBracket[selec_char])) {
                ...
                if (this.findEndBracket(infos, selec_char) === true) {
                    ...
                    _$("end_bracket").innerHTML = (this
                        .assocBracket[selec_char] ||
                        this.revertAssocBracket[selec_char]);
                } else {
            ...
            //alert("move cursor");
            this.displayToCursorPosition("cursor_pos",
                infos["line_start"],
                infos["curr_pos"] - 1,
                infos["curr_line"],
                no_real_move);
...
// count missing empty \r to the selection
        selectionStart -= (line_start - this.textarea.value.substr(0, selectionStart).split("\n").length) * 2;
...
EditArea.prototype.tab_selection = function() {
    ...
    if (this.isIE) {
        this.setIESelection();
        setTimeout("editArea.is_tabbing=false;", 100);
// IE can't accept to make 2 tabulation without a little break between both
    } else {
...
EditArea.prototype.invert_tab_selection = function() {
    ...
    if (t.isIE) {
        ...
        t.setIESelection();
        setTimeout("editArea.is_tabbing=false;", 100);
// IE can accept to make 2 tabulation without a little break between both
    } else
        t.is_tabbing = false;
};
...
EditArea.prototype.findEndBracket = function(infos, bracket) {
    ...
    this.displayToCursorPosition("end_bracket",
        line,
        curPos,
...
EditArea.prototype.displayToCursorPosition = function(id, start_line, cur_pos, lineContent, no_real_move) {
    ...
    content = "<span id='test_font_size_inner'>" +
        lineContent.substr(0, cur_pos).replace(/&/g, "&amp;").replace(/</g, "&lt;") +
    ...
    posTop = this
        .getLinePosTop(start_line) +
        topOffset; // + Math.floor( ( endElem.offsetHeight - 1 ) / this.lineHeight ) * this.lineHeight;
