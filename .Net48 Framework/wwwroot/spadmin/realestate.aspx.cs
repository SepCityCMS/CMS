...
namespace wwwroot
{
    ...
    public partial class realestate : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[3].Text = SepFunctions.LangText("Mark as Pending");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Title");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Property Type");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("City");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("State");
                    ManageGridView.Columns[6].HeaderText = SepFunctions.LangText("Status");
                }
            }
        }
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    switch (FilterDoAction.Value)
                    {
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.RealEstate.Property_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.RealEstate.Property_Change_Status(sIDs, 0);
                            break;
                    }
                }
                catch
        ...
        {
            var dRealEstateProperties = SepCommon.DAL.RealEstate.GetRealEstateProperties(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);
