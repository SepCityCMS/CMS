...
namespace SepCityCMS.Server.Controls
{
    ...
    public class UserTopMenu
    {
        ...
        public string Render()
        {
            ...
            ModuleID = SepFunctions.toInt(SepCore.Session.getSession("ModuleID"));

            var CategoryID = SepFunctions.toLong(SepCore.Request.Item("CatID"));
            ...
            switch (ModuleID)
            {
                ...
                case 35:
                    ...
                    if (!string.IsNullOrWhiteSpace(SepCore.Request.Item("ArticleID")))
                    {
                        output.AppendLine("<li class=\"nav-item\"><a class=\"nav-link\" href=\"" + sInstallFolder + "article_display.aspx?DoAction=Print&ArticleID=" + SepFunctions.UrlEncode(SepCore.Request.Item("ArticleID")) + "\" target=\"_blank\"><i class=\"fa fa-print\"></i> " + SepFunctions.LangText("Print this Article") + "</a></li>");
                    }

                    if (!string.IsNullOrWhiteSpace(SepCore.Request.Item("ArticleID")) && SepFunctions.CompareKeys(GetManageKeys, true))
                    {
                        output.AppendLine("<li class=\"nav-item\"><a class=\"nav-link\" href=\"" + sInstallFolder + "articles_modify.aspx?ArticleID=" + SepFunctions.UrlEncode(SepCore.Request.Item("ArticleID")) + "\"><i class=\"fa fa-plus\"></i> " + SepFunctions.LangText("Edit Article") + "</a></li>");
                    }
                ...
                case 46:
                    ...
                    output.AppendLine("<li class=\"nav-item\"><a class=\"nav-link\" href=\"" + sInstallFolder + "events_modify.aspx?EventDate=" + SepFunctions.UrlEncode(SepCore.Request.Item("EventDate")) + "\"><i class=\"fa fa-plus\"></i> " + SepFunctions.LangText("Post Event") + "</a></li>");
                    if (SepFunctions.Setup(993, "RSSTop") == "Yes")
                    {
                ...
                case 999:
                    if (!string.IsNullOrWhiteSpace(SepCore.Request.Item("UniqueID")))
                    {
                        if (SepFunctions.CompareKeys(SepFunctions.Security("AdminEditPage"), true))
                        {
                            if (SepFunctions.Get_Portal_ID() == 0)
                            {
                                using (SqlConnection conn = new SqlConnection(SepFunctions.Database_Connection()))
                                {
                                    ...
                                    {
                                        cmd.Parameters.AddWithValue("@UniqueID", SepCore.Request.Item("UniqueID"));
                                        using (SqlDataReader RS = cmd.ExecuteReader())
                                        {
                                            if (RS.HasRows)
                                            {
            ...
            if (ModuleID != 33 && showMenu && ModuleID != 17)
            {
                ...
                {
                    output.AppendLine("<li class=\"nav-item\"><a class=\"nav-link\" href=\"" + sInstallFolder + "refer.aspx?URL=" + SepFunctions.UrlEncode("/" + GetPageName + SepCore.Request.Url.Query()) + "&ModuleID=" + ModuleID + "\"><i class=\"fa fa-envelope\"></i> " + SepFunctions.LangText("Refer a Friend") + "</a></li>");
                }
                ...
                if (SepFunctions.Setup(993, "FavoritesTop") != "No")
                {
                    if (!string.IsNullOrWhiteSpace(SepFunctions.Session_User_Name()))
                    {
                        ...
                        saveButton = "<button type=\"button\" class=\"btn btn-primary\" data-dismiss=\"saveButtonmodal\" onclick=\"saveFavorite('" + CategoryID + "', '" + ModuleID + "', '" + SepFunctions.UrlEncode(GetPageName + Strings.ToString(!string.IsNullOrWhiteSpace(SepCore.Request.Url.Query()) ? SepCore.Request.Url.Query() + "&" : "?") + "ModuleID=" + ModuleID) + "');$(this).hide();\">" + SepFunctions.LangText("Save") + "</button>";
