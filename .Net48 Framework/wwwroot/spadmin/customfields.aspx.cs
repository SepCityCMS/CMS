...
// ***********************************************************************
// <copyright file="customfields.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class customfields : Page
    {
        ...
        public string Format_Field(string AnswerType)
        {
            switch (AnswerType)
            {
                case "ShortAnswer":
                ...
                case "DropdownS":
                    return "Dropdown (Single Selection)";
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Fields");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Field Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Answer Type");
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No custom fields have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.CustomFields.Field_Delete(sIDs);
        ...
        {
            var dCustomFields = SepCommon.DAL.CustomFields.GetCustomFields(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value, SepFunctions.toInt(SepCommon.SepCore.Request.Item("ModuleID")));

            dv = new DataView(SepFunctions.ListToDataTable(dCustomFields));
            return dv;
        }
