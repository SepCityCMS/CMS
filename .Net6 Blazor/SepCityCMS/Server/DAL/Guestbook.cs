...
namespace SepCityCMS.Server.DAL
{
    ...
    public static class Guestbook
    {
        ...
        public static string Entry_Delete(string EntryIDs)
        {
            ...
            sReturn = SepFunctions.LangText("Guestbook Entry(s) has been successfully deleted.");
        ...
        public static List<GuestbookEntries> GetGuestbookEntries(string SortExpression = "DatePosted", string SortDirection = "DESC", string searchWords = "")
        {
            ...
            using (var ds = new DataSet())
            {
                using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
                {
                    using (var cmd = new SqlCommand("SELECT G.EntryID,G.Message,G.DatePosted,G.SiteURL,M.Username FROM Guestbook AS G,Members AS M WHERE G.UserID=M.UserID AND G.PortalID=@PortalID AND G.Status <> -1" + wClause + " ORDER BY " + SortExpression + " " + SortDirection, conn))
                    {
