...
namespace SepCityCMS.Server.Integrations
{
    ...
    public class PCRecruiter : IDisposable
    {
        ...
        public XmlNodeList GetFieldNodes(PCR_RecordType recordType, string xAttribute, string xAttributeValue, bool hasSetting)
        {
            ...
            if (File.Exists(Server.SepFunctions.GetDirValue("app_data") + "pcrfields.xml"))
            {
                var doc = new XmlDocument();
                doc.Load(Server.SepFunctions.GetDirValue("app_data") + "pcrfields.xml");
        ...
        {
            return Server.SepFunctions.Setup(ModuleID, "PCRAPIURL");
        }
        ...
        public string GetSessionId()
        {
            ...
            var sSiteName = Server.SepFunctions.Setup(992, "WebSiteName");
            ...
            {
                WRequest = (HttpWebRequest)WebRequest.Create(GetPCRequiterURL() + "access-token?username=" + Server.SepFunctions.Setup(ModuleID, "PCRUserName") + "&password=" + Server.SepFunctions.Decrypt(Server.SepFunctions.Setup(ModuleID, "PCRPassword")) + "&databaseid=" + Server.SepFunctions.Setup(66, "PCRDatabaseId") + "&AppId=" + Server.SepFunctions.Setup(66, "PCRAppId") + "&ApiKey=" + Server.SepFunctions.Setup(66, "PCRAPIKey"));
        ...
        public void Members2PCR(bool force = false)
        {
            ...
            if ((string.IsNullOrWhiteSpace(Server.SepFunctions.GetUserInformation("PCRCandidateId")) || Server.SepFunctions.GetUserInformation("PCRCandidateId") == "0" || force) && !string.IsNullOrWhiteSpace(Server.SepFunctions.Session_User_ID()))
            {
                var cCandidates = new PCRecruiterCandidateFields();
                cCandidates.FirstName = Server.SepFunctions.GetUserInformation("FirstName");
                cCandidates.LastName = Server.SepFunctions.GetUserInformation("LastName");
                cCandidates.City = Server.SepFunctions.GetUserInformation("City");
                cCandidates.State = Server.SepFunctions.GetUserInformation("State");
                cCandidates.PostalCode = Server.SepFunctions.GetUserInformation("ZipCode");
                cCandidates.EmailAddress = Server.SepFunctions.GetUserInformation("EmailAddress");
                cCandidates.Country = Server.SepFunctions.GetUserInformation("Country");
                cCandidates.HomePhone = Server.SepFunctions.GetUserInformation("PhoneNumber");
                cCandidates.UserName = "CANDIDATE";
                if (Server.SepFunctions.CompareKeys(Server.SepFunctions.Security("PCREmployer"), false))
                {
                ...
                try
                {
                    ...
                    using (var conn = new SqlConnection(SepCityCMS.Server.SepFunctions.Database_Connection()))
                    {
                        conn.Open();
                        using (var cmd = new SqlCommand("UPDATE Members SET PCRCandidateId=@PCRCandidateId WHERE UserID=@UserID", conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@UserID", Server.SepFunctions.Session_User_ID());
                            cmd.ExecuteNonQuery();
                        }
                    }
                }
