...
namespace wwwroot.BusinessObjects
{
    using SepCommon;
    ...
    public class Replace : IDisposable
    {
        ...
        public string Replace_Widgets(string strPageText, long ModuleID, bool isLatestNews = false)
        {
                    ...
                    using (var cmd = new SqlCommand("SELECT ZoneID,ZoneName FROM TargetZones WHERE ModuleID='2' AND Status <> -1", conn))
                    {
                        using (SqlDataReader RS = cmd.ExecuteReader())
                        {
                            while (RS.Read())
                            {
                                ...
                                {
                                    var cAds = new SepCityControls.AdServer
                                    {
                    ...
                    using (var cmd = new SqlCommand("SELECT ZoneID,ZoneName FROM TargetZones WHERE ModuleID='1' AND Status <> -1", conn))
                    {
                        using (SqlDataReader RS = cmd.ExecuteReader())
                        {
                            while (RS.Read())
                            {
                                ...
                                {
                                    var cCR = new SepCityControls.ContentRotator
                                    {
                                        ZoneID = SepFunctions.openNull(RS["ZoneID"])
                                    };
            ...
            if (Strings.InStr(strPageText, "]]") > 0 && Strings.InStr(strPageText, "[[") > 0)
            {
                ...
                {
                    var cBreadcrumbs = new SepCityControls.Breadcrumbs();
                ...
                {
                    var cUserTopMenu = new SepCityControls.UserTopMenu();
                ...
                {
                    var cNewsletters = new SepCityControls.Newsletters();
                ...
                {
                    var cSearch = new SepCityControls.SearchBox();
                ...
                {
                    var cSiteName = new SepCityControls.CompanyName();
                ...
                if (ModuleID != 7)
                {
                    ...
                    {
                        var cMemberStatistics = new SepCityControls.MemberStatistics();
                    ...
                    {
                        var cListNewMembers = new SepCityControls.ListNewMembers();
                    ...
                    {
                        var cFriendList = new SepCityControls.FriendList();
                    ...
                    {
                        var cUnreadMessages = new SepCityControls.UnreadMessages();
                    ...
                    for (int i = 1; i <= 7; i++)
                    {
                        ...
                        {
                            var cMenu = new SepCityControls.SiteMenu
                            {
                                MenuID = i
                            };
