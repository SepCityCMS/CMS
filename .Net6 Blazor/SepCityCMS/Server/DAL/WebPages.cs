...
namespace SepCityCMS.Server.DAL
{
    ...
    public static class WebPages
    {
        ...
        public static List<Models.WebPages> GetWebPages(long MenuID = 3, string SortExpression = "Weight", string SortDirection = "ASC", string searchWords = "")
        {
            ...
            {
                wClause = " AND (LinkText LIKE '" + Server.SepFunctions.FixWord(searchWords) + "%')";
            }
            ...
            using (var ds = new DataSet())
            {
                ...
                for (var i = 0; i <= ds.Tables[0].Rows.Count - 1; i++)
                {
                    ...
                    if (Server.SepFunctions.ModuleActivated(Server.SepFunctions.toInt(Server.SepFunctions.openNull(ds.Tables[0].Rows[i]["ModuleID"]))))
                    {
                        rowCount += 1;
                        var dWebPages = new Models.WebPages { UniqueID = Server.SepFunctions.toLong(Server.SepFunctions.openNull(ds.Tables[0].Rows[i]["UniqueID"])) };
                        dWebPages.PageID = Server.SepFunctions.toLong(Server.SepFunctions.openNull(ds.Tables[0].Rows[i]["PageID"]));
                        dWebPages.MenuID = Server.SepFunctions.toInt(Server.SepFunctions.openNull(ds.Tables[0].Rows[i]["MenuID"]));
                        dWebPages.LinkText = Server.SepFunctions.openNull(ds.Tables[0].Rows[i]["LinkText"]);
                        dWebPages.Weight = Server.SepFunctions.toInt(Server.SepFunctions.openNull(ds.Tables[0].Rows[i]["Weight"]));
                        if (Server.SepFunctions.toInt(Server.SepFunctions.openNull(ds.Tables[0].Rows[i]["Status"])) == 1)
                        {
        ...
        public static string Save_External_Link(long PageID, long MenuID, string LinkText, string PageURL, string TargetWindow, long PortalID)
        {
                    ...
                    else
                    {
                        using (var cmd = new SqlCommand("SELECT Count(UniqueID) AS Counter FROM ModulesNPages WHERE MenuID=@MenuID AND Status=1", conn))
                        {
                            ...
                            using (SqlDataReader RS = cmd.ExecuteReader())
                            {
                                ...
                                {
                                    RS.Read();
                                    GetNewWeight = Server.SepFunctions.toLong(Server.SepFunctions.openNull(RS["Counter"]));
                                }
                            }
                        }
            ...
            Server.SepFunctions.Cache_Remove();

            var sReturn = Server.SepFunctions.LangText("External link has been successfully saved.");
        ...
        public static string Save_Web_Page(long PageID, long MenuID, string UserID, string LinkText, string PageText, string SEOPageTitle, string SEODescription, string SEOKeywords, string AccessKeys, string ManageKeys, long Status, long CategoryID, int ModuleID, long PortalID, string PortalIDs)
        {
            ...
            using (var conn = new SqlConnection(Server.SepFunctions.Database_Connection()))
            {
                ...
                if (CategoryID > 0 && ModuleID > 0)
                {
                    ...
                    using (var cmd = new SqlCommand(SqlStr, conn))
                    {
                        cmd.Parameters.AddWithValue("@UniqueID", Server.SepFunctions.GetIdentity());
                    ...
                    return Server.SepFunctions.LangText("Web page has been successfully saved.");
                }
                ...
                if (PortalID == 0)
                {
                            ...
                            using (var cmd = new SqlCommand("SELECT UniqueID FROM ModulesNPages WHERE ModuleID=@ModuleID", conn))
                            {
                                ...
                                using (SqlDataReader RS = cmd.ExecuteReader())
                                {
                                    if (RS.HasRows)
                                    {
                                        RS.Read();
                                        bUpdate = true;
                                        PageID = Server.SepFunctions.toLong(Server.SepFunctions.openNull(RS["UniqueID"]));
                                    }
                                }
                            }
                        }
                        else
                        {
                            using (var cmd = new SqlCommand("SELECT PageID,EditKeys FROM ModulesNPages WHERE UniqueID=@UniqueID", conn))
                            {
                                ...
                                using (SqlDataReader RS = cmd.ExecuteReader())
                                {
                                    ...
                                    {
                                        RS.Read();
                                        if (Server.SepFunctions.toLong(Server.SepFunctions.openNull(RS["PageID"])) == 200)
                                        {
                                            PageEditKeys = Server.SepFunctions.openNull(RS["EditKeys"]);
                                        }
                                        else
                                        {
                                            PageEditKeys = Server.SepFunctions.Security("AdminEditPage");
                                        }
                    ...
                    {
                        PageID = Server.SepFunctions.GetIdentity();
                        PageEditKeys = Server.SepFunctions.Security("AdminEditPage");
                        bUpdate = false;
                    }

                    if (Server.SepFunctions.CompareKeys(PageEditKeys, false, UserID) == false)
                    {
                        return Server.SepFunctions.LangText("Sorry you do not have access to edit this web page.");
                    }
                    ...
                    if (bUpdate)
                    {
                        ...
                        if (Status == 1)
                        {
                            LuceneNet.SepFunctions.IndexRecord(995, PageID);
                        }
                    }
                    else
                    {
                        using (var cmd = new SqlCommand("SELECT Count(UniqueID) AS Counter FROM ModulesNPages WHERE MenuID=@MenuID AND Activated='1'", conn))
                        {
                            ...
                            using (SqlDataReader RS = cmd.ExecuteReader())
                            {
                                ...
                                {
                                    RS.Read();
                                    GetNewWeight = Server.SepFunctions.openNull(RS["Counter"]);
                                }
                            }
                        }
            ...
            var sReturn = Server.SepFunctions.LangText("Web page has been successfully saved.");
            return sReturn;
        }
    }
}