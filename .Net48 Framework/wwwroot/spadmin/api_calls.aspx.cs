namespace wwwroot
{
    ...
    public partial class api_calls : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No API calls have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.APICalls.APICallDelete(sIDs);
        ...
        {
            var dAPICalls = SepCommon.DAL.APICalls.GetAPICalls(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dAPICalls));
            return dv;
        }
