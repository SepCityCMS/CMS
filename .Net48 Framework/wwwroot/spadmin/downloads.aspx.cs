...
namespace wwwroot
{
    ...
    public partial class downloads : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Upload(s)");
                    ...
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Title");
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No downloads have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
                ...
                try
                {
                    switch (FilterDoAction.Value)
                    {
                        case "DeleteDownloads":
                            var sDeleteResult = SepCommon.DAL.Downloads.Download_Delete(sIDs);
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.Downloads.Download_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.Downloads.Download_Change_Status(sIDs, 0);
                            break;
                    }
                }
                catch
        ...
        {
            var dDownloads = SepCommon.DAL.Downloads.GetDownloads(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dDownloads));
            return dv;
        }
