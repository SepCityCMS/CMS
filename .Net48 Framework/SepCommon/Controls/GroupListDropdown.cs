            ...
            using (SqlConnection conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SELECT ListID,ListName FROM GroupLists WHERE Status <> -1 ORDER BY ListName", conn))
                {
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        while (RS.Read())
                        {
                            output.AppendLine("<option value=\"" + SepFunctions.openNull(RS["ListID"]) + "\"" + SepCommon.SepCore.Strings.ToString(SepFunctions.openNull(RS["ListID"]) == Text ? " selected=\"selected\"" : string.Empty) + ">" + SepFunctions.openNull(RS["ListName"]) + "</option>");
                        }
                    }
                }
