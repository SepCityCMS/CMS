...
// ***********************************************************************
// <copyright file="contentrotator.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class contentrotator : Page
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No content have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.ContentRotator.Content_Delete(sIDs);
        ...
        {
            var dContentRotator = SepCommon.DAL.ContentRotator.GetContentRotator(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dContentRotator));
            return dv;
        }
