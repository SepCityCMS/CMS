...
// ***********************************************************************
// <copyright file="userprofiles.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class userprofiles : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Profiles");
                    ...
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Views");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Status");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Date Posted");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No profiles have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    ...
                    {
                        case "DeleteProfiles":
                            var sDeleteResult = SepCommon.DAL.UserProfiles.Profile_Delete(sIDs);
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.UserProfiles.Profile_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.UserProfiles.Profile_Change_Status(sIDs, 0);
                            break;
                    }
                }
                catch
        ...
        {
            var dUserProfiles = SepCommon.DAL.UserProfiles.GetUserProfiles(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dUserProfiles));
            return dv;
        }
