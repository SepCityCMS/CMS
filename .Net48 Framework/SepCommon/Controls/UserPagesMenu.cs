        ...
        public string Render()
        {
            ...
            using (SqlConnection conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                if (!string.IsNullOrWhiteSpace(SepFunctions.Setup(7, "UPagesMenu" + MenuID)) && SepFunctions.isUserPage())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SELECT PageID,PageName,PageTitle FROM UPagesPages WHERE MenuID=7" + MenuID + " AND UserID='" + SepFunctions.GetUserID(SepCommon.SepCore.Request.Item("UserName")) + "' ORDER BY Weight,PageName", conn))
                    {
