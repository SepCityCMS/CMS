    ...
    public class PortalDropdown
    {
        ...
        public string Render()
        {
            ...
            using (SqlConnection conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                output.AppendLine("<option value=\"0\"" + SepCommon.Core.SepCore.Strings.ToString("0" == Text ? " selected=\"selected\"" : string.Empty) + ">" + SepFunctions.LangText("Main Portal") + "</option>");
                using (SqlCommand cmd = new SqlCommand("SELECT PortalID,PortalTitle FROM Portals WHERE Status <> -1 ORDER BY PortalTitle", conn))
                {
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        while (RS.Read())
                        {
                            output.AppendLine("<option value=\"" + SepFunctions.openNull(RS["PortalID"]) + "\"" + SepCommon.Core.SepCore.Strings.ToString(SepFunctions.openNull(RS["PortalID"]) == Text ? " selected=\"selected\"" : string.Empty) + ">" + SepFunctions.openNull(RS["PortalTitle"]) + "</option>");
                        }
                    }
                }
