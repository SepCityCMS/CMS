<%@ page title="E-Learning" language="C#" viewstatemode="Enabled" masterpagefile="Site.Master"
    codebehind="elearning_exams.aspx.cs" inherits="wwwroot.elearning_exams" %>
...
<asp:content id="BodyContent" runat="server" contentplaceholderid="MainContent">
    ...
    <asp:Panel ID="UpdatePanel" runat="server">
        ...
        <h2>
            <span ID="PageHeader" runat="server" Text="Manage the E-Learning Exams"></span>
        </h2>
						...
						<div class="input-group">
							<select id="FilterDoAction" runat="server" class="form-control" ClientIDMode="Static">
								<option value="">Select an Action</option>
									<option value="DeleteExams">Delete Exams</option>
                    </select>
							<span class="input-group-btn">
							<button class="btn btn-light" ID="RunAction" runat="server" OnServerClick="RunAction_Click" onclick="if(ExecuteAction(this, 'ExamID') == false) {return false} else">Go!</button>
						</span>
					</div>
				</div>
				<div class="col-lg-6">
            ...
            <asp:GridView ID="ManageGridView" runat="server" AutoGenerateColumns="False" AllowSorting="True" ClientIDMode="Static"
                ...
                <Columns>
                    <asp:TemplateField ItemStyle-Width="20px">
                        ...
                        <ItemTemplate>
                            <input type="checkbox" id="ExamID<%#
                Eval("ExamID").ToString()%>" value="<%#
                Eval("ExamID").ToString()%>" onclick="gridviewSelectRow(this);" />
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:templatefield ItemStyle-Width="20px">
                        <itemtemplate>
                            <a href="elearning_exams_modify.aspx?ExamID=<%#
                Eval("ExamID").ToString()%>">
                ...
                Eval("ExamName").ToString()%>
                    ...
                    <asp:TemplateField HeaderText="Start Date" HeaderStyle-HorizontalAlign="Left" ItemStyle-VerticalAlign="Top" SortExpression="StartDate">
                        <ItemTemplate>
                            <%#
                Format_Date(Eval("StartDate").ToString()).ToString()%>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="End Date" HeaderStyle-HorizontalAlign="Left" ItemStyle-VerticalAlign="Top" SortExpression="EndDate">
                        <ItemTemplate>
                            <%#
                Format_Date(Eval("EndDate").ToString()).ToString()%>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
