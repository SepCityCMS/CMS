    ...
    public partial class vouchers : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Vouchers");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Buy Title");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Purchase Code");
                }
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.Vouchers.Voucher_Delete(sIDs);
        ...
        {
            var dVouchers = SepCommon.DAL.Vouchers.GetVouchers(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);
