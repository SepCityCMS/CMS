...
namespace SepCityCMS.Server.Integrations.Shipping
{
    ...
    public class UPS : IDisposable
    {
        ...
        public decimal GetRate(string weight, string weightType, string shipperZipCode, string shipperCountryCode, string destinationZipCode, string destinationCountryCode, string serviceCode)
        {
            if (DebugMode)
            {
                Server.SepFunctions.Debug_Log("UPS URL: " + Uri);
            }
            ...
            using (var writer = new StreamWriter(req.GetRequestStream()))
            {
                ...
                if (DebugMode)
                {
                    Server.SepFunctions.Debug_Log("UPS Post Data: " + buildUPSXml(weight, weightType, shipperZipCode, shipperCountryCode, destinationZipCode, destinationCountryCode, serviceCode));
                }
            }
            rsp = req.GetResponse();
            ...
            if (DebugMode)
            {
                Server.SepFunctions.Debug_Log("UPS Response: " + rawXml);
            }
