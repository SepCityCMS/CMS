...
// ***********************************************************************
// <copyright file="flows.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot.twilio
{
    ...
    public partial class flows : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (SepCommon.SepCore.Strings.Len(sIDs) > 0)
            {
                try
                {
                    using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
                    {
                        ...
                        if (arrIDs != null)
                        {
                            ...
                            {
                                using (var cmd = new SqlCommand("DELETE FROM TwilioFlows WHERE FlowID=@FlowID", conn))
                                {
                                    cmd.Parameters.AddWithValue("@FlowID", arrIDs[i]);
                                    cmd.ExecuteNonQuery();
                                }
                ...
                DeleteResult.InnerHtml = "<div class=\"alert alert-success\" role=\"alert\">" + SepFunctions.LangText("You have successfully deleted flow(s)") + "</div>";
        ...
        private DataView BindData()
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT * FROM TwilioFlows ORDER BY FlowName", conn))
                {
