    ...
    public class AccountMenu
    {
        ...
        public string Render()
        {
            ...
            else
            {
                ...
                output.AppendLine("<input type=\"hidden\" name=\"Activate2\" value=\"" + SepCore.Request.Item("Activate") + "\" />");
                foreach (var Item in SepCore.Request.Form())
                {
                    if (SepCore.Strings.ToString(Item) != "Activate2" && SepCore.Strings.ToString(Item) != "UserName2" && SepCore.Strings.ToString(Item) != "Password2")
                    {
                        output.AppendLine("<input type=\"hidden\" name=\"" + SepCore.Strings.ToString(Item) + "\" value=\"" + SepFunctions.HTMLEncode(SepCore.Request.Form(SepCore.Strings.ToString(Item))) + "\" />");
                    }
                }
