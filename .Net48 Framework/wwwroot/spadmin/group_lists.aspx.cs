...
// ***********************************************************************
// <copyright file="group_lists.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class group_lists : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Group Lists");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("List Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Description");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Date Posted");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("User Name");
                    ManageGridView.Columns[6].HeaderText = SepFunctions.LangText("View Users");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No group lists have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.GroupLists.List_Delete(sIDs);
        ...
        {
            var dGroupLists = SepCommon.DAL.GroupLists.GetGroupLists(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dGroupLists));
            return dv;
        }
