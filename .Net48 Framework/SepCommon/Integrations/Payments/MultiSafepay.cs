...
namespace SepCommon
{
    ...
    public class MultiSafepay : IDisposable
    {
        ...
        {
            string outgoingTRANSaction = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" + Environment.NewLine;
                    ...
                    using (var cmd = new SqlCommand("SELECT ScriptText FROM Scripts WHERE ScriptType='GATEWAYS'", conn))
                    {
                        using (SqlDataReader RS = cmd.ExecuteReader())
                        {
                            if (RS.HasRows)
                            {
                                ...
                                string sMSP = SepFunctions.ParseXML("MultiSafePay", sXML);
        ...
        public string StartTransaction()
        {
            ...
            string ipAddress = SepCore.Request.ServerVariables("REMOTE_ADDR");
            string forwardedIp = SepCore.Request.ServerVariables("HTTP_X_FORWARDED_FOR");
            byte[] hashedBytes = md5Hasher.ComputeHash(encoder.GetBytes(Transaction.Amount + Transaction.Currency + Merchant.AccountId + Merchant.SiteId + Transaction.ID));
