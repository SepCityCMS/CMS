...
// ***********************************************************************
// <copyright file="shopmall.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class shopmall : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public string Format_Price(string sSalePrice, string sUnitPrice, string sRecurringPrice, string sRecurringCycle)
        {
            ...
            if (SepFunctions.toDecimal(sSalePrice) < SepFunctions.toDecimal(sUnitPrice) && SepFunctions.toDecimal(sSalePrice) > 0)
                str += "<s>";
            str += SepFunctions.Pricing_Long_Price(SepFunctions.toDecimal(sUnitPrice), SepFunctions.toDecimal(sRecurringPrice), sRecurringCycle);
            if (SepFunctions.toDecimal(sSalePrice) < SepFunctions.toDecimal(sUnitPrice) && SepFunctions.toDecimal(sSalePrice) > 0)
                str += "</s>";
        ...
        public string Get_Sale_Percentage(string sSalePrice, string sUnitPrice, string sRecurringPrice)
        {
            if (SepFunctions.toDecimal(sSalePrice) < SepFunctions.toDecimal(sUnitPrice) && SepFunctions.toDecimal(sSalePrice) > 0)
            {
                ...
                var sSetupSave = SepFunctions.toDouble(sRecurringPrice) > 0 ? " " + SepFunctions.LangText("on the setup") : string.Empty;
                return "(-" + SepFunctions.Format_Currency(sSavePrice) + "), " + SepFunctions.LangText("save") + " " + sSavePercent + "%" + sSetupSave;
            }
        ...
        {
            return SepFunctions.UrlEncode(SepCommon.SepCore.Request.RawUrl() + Context.Request.Url.Query);
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            {
                var jShopMall = SepCommon.DAL.ShoppingMall.Product_Get(SepFunctions.toLong(SepCommon.SepCore.Request.Item("ProductID")));
                if (jShopMall.ProductID > 0) SepFunctions.Redirect(sInstallFolder + "shopping/" + SepCommon.SepCore.Request.Item("ProductID") + "/" + Format_ISAPI(jShopMall.ProductName));
            }
        ...
        private DataView BindData()
        {
            ...
            if (cProducts.Count > 0)
            {
                ...
                if (cProducts.Count <= SepFunctions.toLong(SepFunctions.Setup(992, "RecPerAPage"))) PagerTemplate.Visible = false;
            ...
            if (SepFunctions.toLong(SepCommon.SepCore.Request.Item("CatID")) == 0)
            {
                ...
                if (SepFunctions.Setup(GlobalVars.ModuleID, "ShopMallDisplayNewest") == "Yes")
                {
                    var cProductsN = SepCommon.DAL.ShoppingMall.GetShopProducts("Mod.DatePosted", "DESC", showSalesOnly: false, ShowOnlyAvailable: true, UserID: SepFunctions.isUserPage() && SepFunctions.Setup(7, "UPagesTop10") == "Yes" ? SepFunctions.GetUserID(SepCommon.SepCore.Request.Item("UserName")) : string.Empty);

                    NewestContent.DataSource = cProductsN.Take(10);
