...
namespace SepCommon.DAL
{
    ...
    public static class Blogs
    {
        ...
        public static string Blog_Change_Status(string BlogIDs, int Status)
        {
            ...
            return SepFunctions.LangText("Blog(s) status has been successfully saved.");
        }
        ...
        public static string Blog_Delete(string BlogIDs)
        {
            ...
            return SepFunctions.LangText("Blog(s) has been successfully deleted.");
        }
        ...
        public static List<Models.Blogs> GetBlogs(long CategoryId = -1, string SortExpression = "Mod.DatePosted", string SortDirection = "DESC", string searchWords = "", string UserID = "", bool showAvailable = false, string StartDate = "")
        {
            ...
            using (var ds = new DataSet())
            {
                using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
                {
                    using (var cmd = new SqlCommand("SELECT Mod.BlogID,Mod.BlogName,Mod.Message,Mod.Status,Mod.DatePosted,Mod.UserID,M.Username FROM Blog AS Mod,Members AS M" + SepFunctions.Category_SQL_Manage_Select(CategoryId, wClause) + " ORDER BY " + SortExpression + " " + SortDirection, conn))
                    {
                ...
                for (var i = 0; i <= ds.Tables[0].Rows.Count - 1; i++)
                {
                    ...
                    dBlogs.UserID = SepFunctions.openNull(ds.Tables[0].Rows[i]["UserID"]);
