<%@ page title="Link Directory" language="C#" viewstatemode="Enabled" masterpagefile="Site.Master"
    codebehind="linkdirectory.aspx.cs" inherits="wwwroot.linkdirectory" %>
...
<asp:content id="BodyContent" runat="server" contentplaceholderid="MainContent">
    ...
    <asp:Panel ID="UpdatePanel" runat="server">
        ...
        <h2>
            <span ID="PageHeader" runat="server" Text="Manage the Link Directory"></span>
        </h2>
						...
						<div class="input-group">
							<select id="FilterDoAction" runat="server" class="form-control" ClientIDMode="Static">
								<option value="">Select an Action</option>
									<option value="DeleteWebsites">Delete Websites</option>
                        <option value="MarkActive">Mark as Active</option>
                        <option value="MarkPending">Mark as Pending</option>
                    </select>
							<span class="input-group-btn">
							<button class="btn btn-light" ID="RunAction" runat="server" OnServerClick="RunAction_Click" onclick="if(ExecuteAction(this, 'LinkID') == false) {return false} else">Go!</button>
						</span>
					</div>
				</div>
				<div class="col-lg-6">
            ...
            <asp:GridView ID="ManageGridView" runat="server" AutoGenerateColumns="False" AllowSorting="True" ClientIDMode="Static"
                ...
                <Columns>
                    <asp:TemplateField ItemStyle-Width="20px">
                        ...
                        <ItemTemplate>
                            <input type="checkbox" id="LinkID<%#
                Eval("LinkID").ToString()%>" value="<%#
                Eval("LinkID").ToString()%>" onclick="gridviewSelectRow(this);" />
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:templatefield ItemStyle-Width="20px">
                        <itemtemplate>
                            <a href="linkdirectory_modify.aspx?CatID=<%#
                Eval("CatID").ToString()%>&LinkID=<%#
                Eval("LinkID").ToString()%>">
                                <img src="../images/public/edit.png" alt="Edit" />
                            </a>
                        </itemtemplate>
                    </asp:templatefield>
                    <asp:TemplateField HeaderText="Link Name" HeaderStyle-HorizontalAlign="Left" ItemStyle-VerticalAlign="Top" SortExpression="LinkName">
                        <ItemTemplate>
                            <%#
                Eval("LinkName").ToString()%>
                        </ItemTemplate>
                    </asp:TemplateField>
