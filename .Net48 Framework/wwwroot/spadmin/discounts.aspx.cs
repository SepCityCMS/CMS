...
namespace wwwroot
{
    ...
    public partial class discounts : Page
    {
        ...
        /// Formats the price.
        /// </summary>
        /// <param name="MarkOffPrice">The mark off price.</param>
        /// <param name="MarkOffType">Type of the mark off.</param>
        /// <returns>System.String.</returns>
        public string Format_Price(string MarkOffPrice, string MarkOffType)
        {
            switch (MarkOffType)
            {
                case "1":
                ...
                default:
                    return SepFunctions.Format_Currency(MarkOffPrice);
            }
        }
        ...
        /// <returns>System.String.</returns>
        public string Translate_Status(string StatusID, string ExpireDate)
        {
            if (SepFunctions.toDate(ExpireDate) < DateTime.Now)
            {
                return SepFunctions.LangText("Expired");
            }
            else
            {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Discounts");
                    ...
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Label Text");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Company Name");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Price");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Status");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No discounts have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
                ...
                try
                {
                    switch (FilterDoAction.Value)
                    {
                        case "DeleteDiscounts":
                            var sDeleteResult = SepCommon.DAL.Discounts.Discount_Delete(sIDs);
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.Discounts.Discount_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.Discounts.Discount_Change_Status(sIDs, 0);
                            break;
                    }
                }
                catch
        ...
        {
            var dDiscounts = SepCommon.DAL.Discounts.GetDiscounts(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dDiscounts));
            return dv;
        }
