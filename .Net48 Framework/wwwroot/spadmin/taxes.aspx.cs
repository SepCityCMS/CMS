...
// ***********************************************************************
// <copyright file="taxes.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class taxes : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Taxes");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Tax Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Tax Percent");
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No taxes have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.ShoppingCart.Tax_Delete(sIDs);
        ...
        {
            var dTaxes = SepCommon.DAL.ShoppingCart.GetTaxes(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dTaxes));
            return dv;
        }
