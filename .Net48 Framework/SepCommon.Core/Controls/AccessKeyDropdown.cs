                ...
                using (SqlCommand cmd = new SqlCommand("SELECT KeyID,KeyName FROM AccessKeys WHERE Status <> -1" + sAdminMode + " ORDER BY KeyName", conn))
                {
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        if (RS.HasRows)
                        {
                            while (RS.Read())
                            {
                                output.AppendLine("<option value=\"" + SepFunctions.openNull(RS["KeyID"]) + "\"" + SepCommon.Core.SepCore.Strings.ToString(SepFunctions.openNull(RS["KeyID"]) == Text ? " selected=\"selected\"" : string.Empty) + ">" + SepFunctions.openNull(RS["KeyName"]) + "</option>");
                            }
                        }
                    }
                }
