...
namespace SepCityCMS.Server.DAL
{
    ...
    public static class Events
    {
        ...
        public static string Event_Change_Status(string EventIDs, int Status)
        {
            ...
            sReturn = SepFunctions.LangText("Event(s) status has been successfully saved.");
        ...
        public static string Event_Delete(string EventIDs)
        {
            ...
            sReturn = SepFunctions.LangText("Event(s) has been successfully deleted.");
        ...
        public static Models.Events Event_Get(long EventID, long ChangeID = 0)
        {
            ...
            var GetBegTime = string.Empty;
            var GetEndTime = string.Empty;
            string[] arrBegTime = null;
                    ...
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        if (RS.HasRows)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    arrBegTime = Strings.Split(SepFunctions.openNull(RS["BegTime"]), ",");
                                ...
                                {
                                    arrEndTime = Strings.Split(SepFunctions.openNull(RS["EndTime"]), ",");
        ...
        public static string Event_Type_Delete(string TypeIDs)
        {
            ...
            sReturn = SepFunctions.LangText("Event Type(s) has been successfully deleted.");
        ...
        public static int Event_Type_Save(long TypeID, string UserID, string TypeName, string AccessKeys, string WriteKeys, long PortalID)
        {
            ...
            intReturn = SepFunctions.Additional_Data_Save(isNewRecord, 46, Strings.ToString(TypeID), UserID, SepFunctions.GetUserInformation("UserName", UserID), "EventType", "Event type", string.Empty);
        ...
        public static List<Models.Events> GetEvents(string SortExpression = "EventDate", string SortDirection = "DESC", string EventDate = "", string searchWords = "", long EventTypeID = 0, string UserID = "", string StartDate = "")
        {
            ...
            {
                wClause += " AND EventDate BETWEEN '" + SepFunctions.FixWord(Strings.FormatDateTime(SepFunctions.toDate(EventDate), Strings.DateNamedFormat.ShortDate)) + " 00:00:00' AND '" + SepFunctions.FixWord(Strings.FormatDateTime(SepFunctions.toDate(EventDate), Strings.DateNamedFormat.ShortDate)) + " 23:59:59'";
            }
            ...
            {
                wClause += " AND EventDate > '" + StartDate + "'";
            }
            ...
            using (var ds = new DataSet())
            {
                ...
                for (var i = 0; i <= ds.Tables[0].Rows.Count - 1; i++)
                {
                    ...
                    {
                        arrBegTime = Strings.Split(SepFunctions.openNull(ds.Tables[0].Rows[i]["BegTime"]), ",");
                        Array.Resize(ref arrBegTime, 3);
                        GetBegTime = DateTime.Parse(Strings.FormatDateTime(SepFunctions.toDate(arrBegTime[0] + ":" + arrBegTime[1] + " " + arrBegTime[2]), Strings.DateNamedFormat.ShortTime)).ToString(@"hh\:mm tt");
                    }
                    else
                    ...
                    {
                        arrEndTime = Strings.Split(SepFunctions.openNull(ds.Tables[0].Rows[i]["EndTime"]), ",");
                        Array.Resize(ref arrEndTime, 3);
                        GetEndTime = DateTime.Parse(Strings.FormatDateTime(SepFunctions.toDate(arrEndTime[0] + ":" + arrEndTime[1] + " " + arrEndTime[2]), Strings.DateNamedFormat.ShortTime)).ToString(@"hh\:mm tt");
                    }
                    else
