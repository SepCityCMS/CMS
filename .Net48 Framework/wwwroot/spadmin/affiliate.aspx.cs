...
// ***********************************************************************
// <copyright file="affiliate.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class affiliate : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public string Reset_Members(string AffiliateIDs)
        {
            var sResult = SepCommon.DAL.Affiliate.Affiliate_Reset(AffiliateIDs);
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Reset Members");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("User Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("PayPal Email");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Website URL");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Revenue Generated");
                    ManageGridView.Columns[6].HeaderText = SepFunctions.LangText("Payout");
                    ManageGridView.Columns[7].HeaderText = SepFunctions.LangText("Pay User");
                    ManageGridView.Columns[8].HeaderText = SepFunctions.LangText("Downline");
                    ManageGridView.Columns[9].HeaderText = SepFunctions.LangText("Payment History");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No affiliates have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            {
                DeleteResult.InnerHtml = Reset_Members(sIDs);
        ...
        {
            var dAffiliateMembers = SepCommon.DAL.Affiliate.GetAffiliateMembers(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dAffiliateMembers));
            return dv;
        }
