...
// ***********************************************************************
// <copyright file="forms.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class forms : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Forms");
                    FilterDoAction.Items[2].Text = SepFunctions.LangText("Mark Available");
                    FilterDoAction.Items[3].Text = SepFunctions.LangText("Mark Unavailable");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Title");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Available");
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No forms have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    ...
                    {
                        case "MarkAvailable":
                            DeleteResult.InnerHtml = "<div class=\"alert alert-success\" role=\"alert\">" + SepCommon.DAL.Forms.Form_Mark_Available(sIDs) + "</div>";
                            break;

                        case "MarkUnavailable":
                            DeleteResult.InnerHtml = "<div class=\"alert alert-success\" role=\"alert\">" + SepCommon.DAL.Forms.Form_Mark_Unavailable(sIDs) + "</div>";
                            break;

                        default:
                            DeleteResult.InnerHtml = "<div class=\"alert alert-success\" role=\"alert\">" + SepCommon.DAL.Forms.Form_Delete(sIDs) + "</div>";
                            break;
                    }
                }
                catch
        ...
        {
            var dForms = SepCommon.DAL.Forms.GetForms(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);
