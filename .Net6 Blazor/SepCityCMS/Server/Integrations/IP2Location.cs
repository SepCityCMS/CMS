        ...
        public static void IP2Location(string strIPAddress, ref string City, ref string State, ref string Country)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (Server.SepFunctions.DebugMode)
                    {
        ...
        public static string SepCityToken()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var sDomain = Server.SepFunctions.GetMasterDomain(false).Replace("://www.", "://");

                    IgnoreBadCertificates();

                    var apiKey = string.Empty;
                    if (Server.SepFunctions.isHosted())
                    {
                        apiKey = Server.SepFunctions.AES_Encrypt(sDomain + "||" + Server.SepFunctions.GetEncryptionKey(), "8695vGPjDlbG5vGP");
                    }
                    else
                    {
                        apiKey = Server.SepFunctions.AES_Encrypt(sDomain + "||" + Server.SepFunctions.GetEncryptionKey() + "||" + Server.SepFunctions.Setup(70, "SepCityAPIKey") + "||" + Server.SepFunctions.Setup(70, "SepCityUser") + "||" + Server.SepFunctions.Setup(70, "SepCityPassword"), "8695vGPjDlbG5vGP");
                    }

                    var url = "https://www.sepcity.com/api/AccessToken?apikey=" + Server.SepFunctions.UrlEncode(apiKey);
                    if (Server.SepFunctions.DebugMode)
                    {
                        url = "https://new.sepcity.com/api/AccessToken?apikey=" + Server.SepFunctions.UrlEncode(apiKey);
                    }
