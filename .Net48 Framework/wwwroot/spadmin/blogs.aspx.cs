...
namespace wwwroot
{
    ...
    public partial class blogs : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Blog Name");
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    switch (FilterDoAction.Value)
                    {
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.Blogs.Blog_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.Blogs.Blog_Change_Status(sIDs, 0);
                            break;
                    }
                }
                catch
        ...
        {
            var dBlogs = SepCommon.DAL.Blogs.GetBlogs(-1, GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);
