...
// ***********************************************************************
// <copyright file="group_lists_users.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    /// <summary>
    /// Class group_lists_users.
    /// Implements the <see cref="System.Web.UI.Page" />
    /// </summary>
    /// <seealso cref="System.Web.UI.Page" />
    public partial class group_lists_users : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete User(s) from Group List");
                    FilterDoAction.Items[2].Text = SepFunctions.LangText("Delete User(s) from Database");
                    ...
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Location");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Date Added");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No users have been added to this group list.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            {
                if (FilterDoAction.Value == "DeleteUserList")
                    try
                    {
                        var sDeleteResult = SepCommon.DAL.GroupLists.Group_Users_Delete(SepFunctions.toLong(ListID.Value), sIDs);
                ...
                if (FilterDoAction.Value == "DeleteUserDatabase")
                    try
                    {
                        var sDeleteResult = SepCommon.DAL.Members.Member_Delete(sIDs);
                        DeleteResult.InnerHtml = Strings.InStr(sDeleteResult, SepFunctions.LangText("Successfully")) > 0 ? "<div class=\"alert alert-success\" role=\"alert\">" + sDeleteResult + "</div>" : "<div class=\"alert alert-danger\" role=\"alert\">" + sDeleteResult + "</div>";
                    }
                    catch
                    {
        ...
        {
            var dGroupListsUsers = SepCommon.DAL.GroupLists.GetGroupListsUsers(SepFunctions.toLong(ListID.Value), GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dGroupListsUsers));
            return dv;
        }
