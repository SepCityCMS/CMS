...
namespace SepCityControls
{
    ...
    public class SiteMenu
    {
        ...
        {
            get => SepCommon.SepCore.Strings.ToString(m_OverrideLinkClass);
        ...
        public string Render()
        {
            ...
            if (DisableUL == false)
            {
                ...
                {
                    str.AppendLine("<ul class=\"navbar-nav" + CssClass + "\">");
                }
            }
            ...
            using (SqlConnection conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                if (SepFunctions.isUserPage())
                {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand("SELECT PageID,PageName,PageTitle FROM UPagesPages WHERE MenuID=" + MenuID + " AND UserID='" + SepFunctions.GetUserID(SepCommon.SepCore.Request.Item("UserName")) + "' ORDER BY Weight,PageName", conn))
                        {
                ...
                else
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT MP.UniqueID,MP.PageID FROM PortalPages AS MP WHERE (MP.PortalIDs LIKE '%|" + SepFunctions.Get_Portal_ID() + "|%' OR MP.PortalIDs LIKE '%|-1|%') AND MP.MenuID=" + MenuID + " AND MP.Status=1", conn))
                    {
                        using (SqlDataReader RS = cmd.ExecuteReader())
                        {
                            while (RS.Read())
                            {
                                if (SepCommon.SepCore.Strings.InStr(sCheckDup, "|" + SepFunctions.openNull(RS["PageID"]) + "|") == 0)
                                {
                    ...
                    if (SepCommon.SepCore.Strings.Len(sId) > 0)
                    {
                        wc = " UniqueID IN (" + SepCommon.SepCore.Strings.Right(sId, SepCommon.SepCore.Strings.Len(sId) - 1) + ")";
                    }
            ...
            output.Append(SepCommon.SepCore.Strings.ToString(str));
        ...
        public string REP_Site_Menu_Loop(string ID, long PageID, long ModuleID, string UserPageName, string TargetWindow, string LinkText)
        {
            ...
            {
                case 201:
                    if (SepCommon.SepCore.Strings.Left(UserPageName, 2) == "~/" || (SepCommon.SepCore.Strings.Left(UserPageName, 1) != "/" && SepCommon.SepCore.Strings.Left(UserPageName, 4) != "www." && SepCommon.SepCore.Strings.Left(UserPageName, 5) != "http:" && SepCommon.SepCore.Strings.Left(UserPageName, 6) != "https:"))
                    {
                        GetPageName = sInstallFolder + SepCommon.SepCore.Strings.Replace(UserPageName, "~/", string.Empty);
                    }
                    else if (SepCommon.SepCore.Strings.Left(UserPageName, 4) == "www.")
                    {
            ...
            return SepCommon.SepCore.Strings.ToString(str);
        }
    }
}