...
namespace SepCityCMS.Server.LuceneNet
{
    ...
    public static partial class SepFunctions
    {
        ...
        private static void AddTextToIndex(int ModuleID, long recordId, string title, string description, string keywords, string html, string url, bool isHTML, string postalCode, string userId)
        {
            ...
            var indexDirectory = Server.SepFunctions.GetDirValue("app_data") + "lucene\\";
            ...
            if (ModuleID > 0)
            {
                indexDirectory = Server.SepFunctions.GetDirValue("app_data") + "lucene\\" + ModuleID + "\\";
            ...
            catch
            {
                ...
                {
                    Server.SepFunctions.Debug_Log("AddTextToIndex: " + ex.Message);
                }
            }
        ...
        private static void AddToFeed(int ModuleID, long recordId, string title, string description, string url, string userId)
        {
            ...
            {
                case 5:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has added a new discount.") + " (" + title + ")";
                    SqlStr = "SELECT DatePosted FROM DiscountSystem WHERE DiscountID=@UniqueID";
                    break;

                case 10:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has uploaded a new file.") + " (" + title + ")";
                    SqlStr = "SELECT DatePosted FROM LibrariesFiles WHERE FileID=@UniqueID";
                    break;

                case 12:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has added a new topic to the forums.") + " (" + title + ")";
                    SqlStr = "SELECT DatePosted FROM ForumsMessages WHERE TopicID=@UniqueID";
                    break;

                case 19:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has added a link to the link directory.") + " (" + title + ")";
                    SqlStr = "SELECT DatePosted FROM LinksWebsites WHERE LinkID=@UniqueID";
                    break;

                case 18:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has updated their profile.");
                    SqlStr = "SELECT DatePosted FROM MatchMaker WHERE ProfileID=@UniqueID";
                    break;

                case 20:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has added a business.") + " (" + title + ")";
                    SqlStr = "SELECT DatePosted FROM BusinessListings WHERE BusinessID=@UniqueID";
                    break;

                case 31:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has added an item for auction.") + " (" + title + ")";
                    SqlStr = "SELECT DatePosted FROM AuctionAds WHERE AdID=@UniqueID";
                    break;

                case 35:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has added an article.") + " (" + title + ")";
                    SqlStr = "SELECT Start_Date FROM Articles WHERE ArticleID=@UniqueID";
                    break;

                case 44:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has added a new classified ad.") + " (" + title + ")";
                    SqlStr = "SELECT DatePosted FROM ClassifiedsAds WHERE AdID=@UniqueID";
                    break;

                case 46:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has added a event.") + " (" + title + ")";
                    SqlStr = "SELECT EventDate FROM EventCalendar WHERE EventID=@UniqueID";
                    break;

                case 61:
                    sTitle = Server.SepFunctions.LangText("~~" + Server.SepFunctions.GetUserInformation("UserName", userId) + "~~ has added a new blog.") + " (" + title + ")";
            ...
            if (!string.IsNullOrWhiteSpace(sTitle))
            {
                using (var conn = new SqlConnection(Server.SepFunctions.Database_Connection()))
                {
                    ...
                    using (var cmd = new SqlCommand(SqlStr, conn))
                    {
                        ...
                        using (SqlDataReader RS = cmd.ExecuteReader())
                        {
                            if (RS.HasRows)
                            {
                                ...
                                if (ModuleID == 46)
                                {
                                    DatePosted = Server.SepFunctions.toDate(Server.SepFunctions.openNull(RS["EventDate"]));
                                }
                                else if (ModuleID == 35)
                                {
                                    DatePosted = Server.SepFunctions.toDate(Server.SepFunctions.openNull(RS["Start_Date"]));
                                }
                                else
                                {
                                    DatePosted = Server.SepFunctions.toDate(Server.SepFunctions.openNull(RS["DatePosted"]));
                                }
                            }
                        }
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(userId))
                    {
                        using (var cmd = new SqlCommand(SqlStr, conn))
                        {
                            cmd.Parameters.AddWithValue("@FeedID", Server.SepFunctions.GetIdentity());
                            cmd.Parameters.AddWithValue("@UniqueID", recordId);
                            cmd.Parameters.AddWithValue("@ModuleID", ModuleID);
        ...
        private static void DeleteTextToIndex(int ModuleID, long recordId)
        {
            ...
            try
            {
                ...
                if (reader.NumDocs > 0)
                {
                    ...
                    recid.Bytes = SepFunctions.StringToBytes(Strings.ToString(recordId));
    ...
    public class LuceneSearch
    {
        ...
        public DataTable search(string Distance = "ANY", string PostalCode = "", string UserID = "")
        {
            ...
            for (var i = startAt; i <= resultsCount - 1; i++)
            {
                ...
                {
                    ShowRecord = true;
                    if (Distance != "ANY" && SepFunctions.toDouble(Integrations.SepFunctions.PostalCodeDistance(PostalCode, doc.Get("postalcode"))) < SepFunctions.toDouble(Distance))
                    {
                        ShowRecord = true;
                    }
                    ...
                    if (ShowRecord)
                    {
                        ...
                        row["url"] = Strings.Replace(Strings.Replace(Strings.Replace(SepFunctions.GetMasterDomain(true) + doc.Get("url"), "http://", string.Empty), "https://", string.Empty), "//", "/");
