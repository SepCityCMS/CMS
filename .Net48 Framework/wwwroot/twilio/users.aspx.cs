...
// ***********************************************************************
// <copyright file="users.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot.twilio
{
    ...
    public partial class users : Page
    {
        ...
        /// <returns>System.String.</returns>
        public string List_Groups(string GroupIDs)
        {
            string sGroups = string.Empty;
            using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                conn.Open();
                string[] arrGroupIDs = null;
                arrGroupIDs = SepCommon.SepCore.Strings.Split(GroupIDs, ",");
                if (arrGroupIDs != null)
                {
                    for (var i = 0; i <= SepCommon.SepCore.Information.UBound(arrGroupIDs); i++)
                    {
                        using (var cmd = new SqlCommand("SELECT * FROM TwilioGroups WHERE GroupID=@GroupID", conn))
                        {
                            cmd.Parameters.AddWithValue("@GroupID", arrGroupIDs[i]);
                            using (SqlDataReader RS = cmd.ExecuteReader())
                            {
                                if (RS.HasRows)
                                {
                                    RS.Read();
                                    sGroups += SepFunctions.openNull(RS["GroupName"]) + "<br/>";
                                }
                ...
                try
                {
                    using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
                    {
                        ...
                        if (arrIDs != null)
                        {
                            ...
                            {
                                using (var cmd = new SqlCommand("DELETE FROM TwilioUsers WHERE UserID=@UserID", conn))
                                {
                                    cmd.Parameters.AddWithValue("@UserID", arrIDs[i]);
                                    cmd.ExecuteNonQuery();
                                }
                            }
                        }
        ...
        private DataView BindData()
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT TU.UserID, Members.UserName, Members.FirstName + ' ' + Members.LastName AS FullName, PhoneNumber, TU.GroupIDs FROM TwilioUsers AS TU, Members WHERE Members.UserID=TU.UserID ORDER BY Members.UserName", conn))
                {
