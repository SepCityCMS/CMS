    ...
    public class AccountMenu
    {
        ...
        public string Render()
        {
            ...
            else
            {
                ...
                output.AppendLine("<input type=\"hidden\" name=\"Activate2\" value=\"" + SepCommon.Core.SepCore.Request.Item("Activate") + "\" />");
                foreach (var Item in SepCommon.Core.SepCore.Request.Form())
                {
                    if (SepCommon.Core.SepCore.Strings.ToString(Item) != "Activate2" && SepCommon.Core.SepCore.Strings.ToString(Item) != "UserName2" && SepCommon.Core.SepCore.Strings.ToString(Item) != "Password2")
                    {
                        output.AppendLine("<input type=\"hidden\" name=\"" + SepCommon.Core.SepCore.Strings.ToString(Item) + "\" value=\"" + SepFunctions.HTMLEncode(SepCommon.Core.SepCore.Request.Form(SepCommon.Core.SepCore.Strings.ToString(Item))) + "\" />");
                    }
                }
            ...
            {
                var CartDisplayStatement = string.Empty;
                if (!string.IsNullOrWhiteSpace(SepFunctions.Session_User_ID()))
                {
