...
namespace SepCommon
{
    ...
    public static partial class SepFunctions
    {
        ...
        public static int Additional_Data_Save(bool isNewRecord, int iModuleID, string RecordID, string sUserID, string Username, string actType, string actDescription, string PointVariable)
        {
            ...
            {
                long iGetPoints = Points_Setup("Get" + PointVariable);
                long iPostPoints = Points_Setup("Post" + PointVariable);
                if (!string.IsNullOrWhiteSpace(sUserID) && (iGetPoints != 0 || iPostPoints != 0))
                {
                    using (var conn = new SqlConnection(Database_Connection()))
                    {
                ...
                using (var cmd = new SqlCommand("SELECT IP.*,(SELECT ModelNumber FROM ShopProducts WHERE ProductID=IP.ProductID) AS ModelNumber,(SELECT ShipOption FROM ShopProducts WHERE ProductID=IP.ProductID) AS ShipOption,INV.UserID,INV.RecurringID,INV.AffiliateID,INV.OrderDate FROM Invoices AS INV,Invoices_Products AS IP WHERE INV.InvoiceID=IP.InvoiceID AND INV.InvoiceID=@InvoiceID AND INV.Status='0'", conn))
                {
                    ...
                    using (SqlDataReader PaidRS = cmd.ExecuteReader())
                    {
                        if (PaidRS.HasRows)
                        {
                            ...
                            while (PaidRS.Read())
                            {
                                //PayPal.CreateOrder(openNull(PaidRS["UserID"]), SepCore.Strings.ToString(sInvoiceID), "99.95", "Affiliate Payout");
                                //SepCommon.DAL.Affiliate.Affiliate_Reset(SepFunctions.GetUserInformation("AffiliateID", openNull(PaidRS["UserID"])));
                                ...
                                if (!String.IsNullOrWhiteSpace(SepFunctions.Setup(989, "PayPalClientID")) && !String.IsNullOrWhiteSpace(SepFunctions.Setup(989, "PayPalSecret")))
                                {
                                    //PayPal.CreateOrder(openNull(PaidRS["UserID"]));
                                }
                                ...
                                switch (toInt(openNull(PaidRS["ModuleID"])))
                                {
                                    ...
                                    case 37:
                                        ...
                                        // E-Learning Module
                                        using (var cmd2 = new SqlCommand("UPDATE ELearnStudents SET Active='1' WHERE Active='0' AND CourseID=@CourseID AND UserID=@UserID", conn))
                                        {
                                            cmd2.Parameters.AddWithValue("@CourseID", SepFunctions.toLong(openNull(PaidRS["ProductID"])));
                                            cmd2.Parameters.AddWithValue("@UserID", openNull(PaidRS["UserID"]));
                                            cmd2.ExecuteNonQuery();
                                        }
                                    ...
                                    case 38:
                                        string[] arrSplit = Strings.Split(openNull(PaidRS["ModelNumber"]), "-");
                                        Array.Resize(ref arrSplit, 2);
                                        PUB_Update_Class(arrSplit[1], openNull(PaidRS["UserID"]));
                                        break;

                                    case 41:
                                        using (var cmd2 = new SqlCommand("SELECT APICalls.* FROM ShopProducts, APICalls WHERE APICalls.APIID=ShopProducts.APIID AND APICalls.Status=1 AND ShopProducts.ShipOption='api' AND ShopProducts.ProductID=@ProductID", conn))
                                        {
                                            cmd2.Parameters.AddWithValue("@ProductID", SepFunctions.toLong(openNull(PaidRS["ProductID"])));
                                            using (SqlDataReader ProductRS = cmd2.ExecuteReader())
                                            {
                                                if (ProductRS.HasRows)
                                                {
                                                    ProductRS.Read();
                                                    try
                                                    {
                                                        var baseUrl = SepFunctions.openNull(ProductRS["ApiURL"]);
                                                        ...
                                                        if (!string.IsNullOrWhiteSpace(SepFunctions.openNull(ProductRS["ApiHeaders"])))
                                                        {
                                                            string[] result = SepFunctions.openNull(ProductRS["ApiHeaders"]).Split(new string[] { "\n", "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
                                                            foreach (string s in result)
                                                            {
                                                                if (s.Split(':').Length > 0)
                                                                {
                                                                    switch (s.Split(':')[0].Trim().ToLower())
                                                                    {
                                                                        case "content-type":
                                                                            _client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue(s.Split(':')[1].Trim()));
                                                                            break;
                                                                        case "authorization":
                                                                            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("bearer", s.Split(':')[1].Trim());
                                                                            break;
                                                                        default:
                                                                            _client.DefaultRequestHeaders.Add(s.Split(':')[0].Trim(), s.Split(':')[1].Trim());
                                                                            break;
                                                                    }
                                                                }
                                                            }
                                                            switch (SepFunctions.openNull(ProductRS["Method"]))
                                                            {
                                                                case "DELETE":
                                                                    _client.DeleteAsync(SepFunctions.openNull(ProductRS["ApiBody"]));
                                                                    break;

                                                                case "PUT":
                                                                    var jsonString = JsonConvert.SerializeObject(SepFunctions.openNull(ProductRS["ApiBody"]));
                                                                    var content = new StringContent(jsonString, Encoding.UTF8, "application/json");
                                                                    _client.PutAsync(baseUrl, content);
                                                                    break;

                                                                case "POST":
                                                                    var jsonString2 = JsonConvert.SerializeObject(SepFunctions.openNull(ProductRS["ApiBody"]));
                                                                    var content2 = new StringContent(jsonString2, Encoding.UTF8, "application/json");
                                                                    _client.PostAsync(baseUrl, content2);
                                                                    break;

                                                                default:
                                                                    _client.GetAsync(SepFunctions.openNull(ProductRS["ApiBody"]));
                                                                    break;
                                                            }
                                                        }
                                                    }
                                                    ...
                                                    {
                                                        SepFunctions.Debug_Log("Error Writing to API: " + ex.Message);
                                                    }
                                                }
                                            }
                                        }
            ...
            using (var conn = new SqlConnection(Database_Connection()))
            {
                ...
                using (var cmd = new SqlCommand("SELECT * FROM AccessClasses WHERE ClassID=@ClassID", conn))
                {
                    ...
                    using (SqlDataReader ClassRS = cmd.ExecuteReader())
                    {
                        if (ClassRS.HasRows)
                        {
                            string sActDesc = LangText("[[Username]] access class has been changed") + Environment.NewLine;
            ...
            for (var i = 1; i <= iQuantity; i++)
            {
                using (var cmd = new SqlCommand("SELECT * FROM Vouchers WHERE VoucherID='" + FixWord(sVoucherId) + "'", Conn))
                {
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        ...
                        {
                            RS.Read();
                            string purchaseCode = openNull(RS["PurchaseCode"]) + "-" + random.Next(1, 9) + Convert.ToInt32(26 * random.NextDouble() + 65) + random.Next(1, 9) + Convert.ToInt32(26 * random.NextDouble() + 65) + random.Next(1, 9) + Convert.ToInt32(26 * random.NextDouble() + 65);
