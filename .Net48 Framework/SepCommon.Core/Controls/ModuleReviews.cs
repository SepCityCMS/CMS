...
namespace SepCityControls.Core
{
    ...
    public class ModuleReviews
    {
        ...
        {
            get => SepCommon.Core.SepCore.Strings.ToString(m_UserID);
                            ...
                            while (RS.Read())
                            {
                                ...
                                using (SqlCommand cmd2 = new SqlCommand("SELECT * FROM ReviewsUsers WHERE ReviewID=@ReviewID AND UserID=@UserID", conn))
                                {
                                    ...
                                    using (SqlDataReader RS2 = cmd2.ExecuteReader())
                                    {
                                        ...
                                        {
                                            RS2.Read();
                                            TotalStars = SepFunctions.toDouble(SepFunctions.openNull(RS2["TotalStars"])) / SepFunctions.toDouble(SepFunctions.openNull(RS2["TotalUsers"]));
