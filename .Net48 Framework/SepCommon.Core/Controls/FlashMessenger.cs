...
namespace SepCityControls.Core
{
    ...
    public class FlashMessenger
    {
        ...
        public string Render()
        {
            ...
            if (!string.IsNullOrWhiteSpace(SepFunctions.Session_User_Name()) && !string.IsNullOrWhiteSpace(SepFunctions.Session_Password()) && string.IsNullOrWhiteSpace(Strings.Replace(Strings.ToString(SepCommon.Core.SepCore.Session.getSession(Strings.Left(Strings.Replace(SepFunctions.Setup(992, "WebSiteName"), " ", string.Empty), 5) + "MessengerOpened")), "'", string.Empty)))
            {
                ...
                SepCommon.Core.SepCore.Session.setSession(Strings.Left(Strings.Replace(SepFunctions.Setup(992, "WebSiteName"), " ", string.Empty), 5) + "MessengerOpened", "true");
            }
