...
// ***********************************************************************
// <copyright file="portals_pages.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class portals_pages : Page
    {
        /// <summary>
        ...
        public string Delete_Web_Pages(string IDs)
        {
            ...
            if (arrIDs != null)
            {
                for (var i = 0; i <= Information.UBound(arrIDs); i++)
                    ...
                    {
                        conn.Open();
                        using (var cmd = new SqlCommand("SELECT UniqueID FROM PortalPages WHERE PortalID=@PortalID AND UniqueID=@UniqueID AND (PageID='200' OR PageID='201')", conn))
                        {
                        ...
                        {
                            using (var cmd = new SqlCommand("UPDATE PortalPages SET Status='-1', DateDeleted=@DateDeleted WHERE PortalID=@PortalID AND UniqueID=@UniqueID AND (PageID='200' OR PageID='201')", conn))
                            {
                                cmd.Parameters.AddWithValue("@UniqueID", arrIDs[i]);
        ...
        protected void ManageGridView_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                if (e.CommandName == "EnableLink")
                {
                    ...
                    {
                        using (var cmd = new SqlCommand("UPDATE PortalPages SET Status=1 WHERE PortalID=@PortalID AND UniqueID=@UniqueID", conn))
                        {
                    ...
                    {
                        using (var cmd = new SqlCommand("UPDATE PortalPages SET Status='0' WHERE PortalID=@PortalID AND UniqueID=@UniqueID", conn))
                        {
                            cmd.Parameters.AddWithValue("@UniqueID", cmdArguments[3]);
                            cmd.Parameters.AddWithValue("@PortalID", SepFunctions.Get_Portal_ID());
                            cmd.ExecuteNonQuery();
                ...
                if (e.CommandName == "MoveUp" || e.CommandName == "MoveDown")
                {
                        ...
                        using (SqlDataReader RS = cmd.ExecuteReader())
                        {
                            while (RS.Read())
                                if (SepFunctions.ModuleActivated(SepFunctions.toInt(SepFunctions.openNull(RS["PageID"]))))
                                {
                                    ...
                                    arrCommands[arrCount] = "UPDATE PortalPages SET Weight='" + SepFunctions.FixWord(Strings.ToString(UpdateWeight)) + "' WHERE PortalID=" + PortalID + " AND UniqueID='" + SepFunctions.openNull(RS["UniqueID"], true) + "' AND MenuID=" + SepFunctions.FixWord(sMenuID) + " AND Status <> -1";
                                    arrCount += 1;
                                }
                    ...
                    if (arrCommands != null)
                    {
                        arrCommands[arrCount] = "UPDATE PortalPages SET Weight='" + SepFunctions.FixWord(Strings.ToString(sNewWeight)) + "' WHERE PortalID=" + PortalID + " AND UniqueID='" + SepFunctions.FixWord(sID) + "' AND UserPageName <> '' AND MenuID=" + SepFunctions.FixWord(sMenuID) + " AND Status <> -1";
                        arrCount += 1;
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT TemplateID FROM SiteTemplates WHERE TemplateID=@TemplateID", conn))
                {
        ...
        {
            var dWebPages = SepCommon.DAL.Portals.GetPortalsPages(MenuID, SepFunctions.Get_Portal_ID(), GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);
