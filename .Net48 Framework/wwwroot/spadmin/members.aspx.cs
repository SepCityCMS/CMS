...
// ***********************************************************************
// <copyright file="members.aspx.cs" company="SepCity, Inc.">
    ...
    public partial class members : Page
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (string.IsNullOrWhiteSpace(SepFunctions.Setup(989, "ZoomAPIKey")) || string.IsNullOrWhiteSpace(SepFunctions.Setup(989, "ZoomAPISecret")))
                {
                    Page.ClientScript.RegisterClientScriptBlock(this.GetType(), "ZoomLink", "<script>$(document).ready(function () {$('[attr-class=ZoomMeetingLink]').hide();});</script>", false);
                }
            }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            {
                if (FilterDoAction.Value == "DeleteMembers") sReturn = Members_Delete(sIDs);
                if (FilterDoAction.Value == "MarkActive") sReturn = Members_Mark_Active(sIDs);
                if (FilterDoAction.Value == "MarkNotActive") sReturn = Members_Mark_Not_Active(sIDs);
                if (FilterDoAction.Value == "MoveClass") sReturn = Members_Move_Class(sIDs);
                if (FilterDoAction.Value == "AddAccessKey") sReturn = Members_Add_Access_Key(sIDs);
                if (FilterDoAction.Value == "RemoveAccessKey") sReturn = Members_Remove_Access_Key(sIDs);
                if (FilterDoAction.Value == "AddGroup") sReturn = Members_Add_Group(sIDs);

                if (!string.IsNullOrWhiteSpace(sReturn)) DeleteResult.InnerHtml = Strings.InStr(sReturn, SepFunctions.LangText("Successfully")) > 0 ? "<div class=\"alert alert-success\" role=\"alert\">" + sReturn + "</div>" : "<div class=\"alert alert-danger\" role=\"alert\">" + sReturn + "</div>";
        ...
        {
            var dMembers = SepCommon.DAL.Members.GetMembers(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);
