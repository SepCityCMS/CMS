...
namespace SepCommon
{
    using SepCommon.RateWebReference;
    using System;
    ...
    public class FedEx : IDisposable //-V3073
    {
        ...
        public RateRequest CreateRateRequest(string weight, string weightType)
        {
            ...
            {
                WebAuthenticationDetail = SetWebAuthenticationDetail(),

                ClientDetail = new ClientDetail
                {
                    AccountNumber = m_accountNumber,
                    MeterNumber = m_meterNumber
                },
        ...
        public decimal GetRate(string weight, string weightType)
        {
            ...
            catch (Exception e)
            {
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("FedEx Error: " + e.Message);
                }
            }
        ...
        public void SetDestination(ref RateRequest request)
        {
            ...
            if (DebugMode)
            {
                SepFunctions.Debug_Log("m_destinationStreet: " + m_destinationStreet);
                SepFunctions.Debug_Log("m_destinationCity: " + m_destinationCity);
                SepFunctions.Debug_Log("m_destinationState: " + m_destinationState);
                SepFunctions.Debug_Log("m_destinationZipCode: " + m_destinationZipCode);
                SepFunctions.Debug_Log("m_destinationCountryCode: " + m_destinationCountryCode);
            }
        }
        ...
        public void SetOrigin(ref RateRequest request)
        {
            request.RequestedShipment.Shipper = new Party
            {
                Address = new Address
                {
                    StreetLines = new string[1] { m_shipperStreet },
                    City = m_shipperCity,
                    StateOrProvinceCode = m_shipperState,
                    PostalCode = m_shipperZipCode,
                    CountryCode = m_shipperCountryCode
                }
            };

            if (DebugMode)
            {
                SepFunctions.Debug_Log("m_shipperStreet: " + m_shipperStreet);
                SepFunctions.Debug_Log("m_shipperCity: " + m_shipperCity);
                SepFunctions.Debug_Log("m_shipperState: " + m_shipperState);
                SepFunctions.Debug_Log("m_shipperZipCode: " + m_shipperZipCode);
                SepFunctions.Debug_Log("m_shipperCountryCode: " + m_shipperCountryCode);
            }
        }
        ...
        public void SetPackageLineItems(string weight, string weightType, ref RateRequest request)
        {
            ...
            if (SepFunctions.toDecimal(weight) > 0 && weightType != "LBS")
            {
                request.RequestedShipment.RequestedPackageLineItems[0].Weight.Value = SepFunctions.toDecimal(weight) / 16;
            }
            else
            {
                request.RequestedShipment.RequestedPackageLineItems[0].Weight.Value = SepFunctions.toDecimal(weight);
            }
        ...
        public void ShowDeliveryDetails(ref RateReplyDetail rateReplyDetail)
        {
            if (rateReplyDetail.DeliveryTimestampSpecified)
            {
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("Delivery timestamp: " + rateReplyDetail.DeliveryTimestamp);
                }
            }
            ...
            if (rateReplyDetail.TransitTimeSpecified)
            {
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("Transit time: " + rateReplyDetail.TransitTime);
                }
            }
        }
        ...
        public void ShowNotifications(ref RateReply reply)
        {
            if (DebugMode)
            {
                SepFunctions.Debug_Log("Notifications");
            }
            ...
            for (var i = 0; i <= reply.Notifications.Length - 1; i++)
            {
                ...
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("Notification no. " + i);
                    SepFunctions.Debug_Log(" Severity: " + notification.Severity);
                    SepFunctions.Debug_Log(" Code: " + notification.Code);
                    SepFunctions.Debug_Log(" Message: " + notification.Message);
                    SepFunctions.Debug_Log(" Source: " + notification.Source);
                }
            }
        }
        ...
        public void ShowRateReply(ref RateReply reply)
        {
            ...
            if (DebugMode)
            {
                SepFunctions.Debug_Log("RateReply details:");
            }
            ...
            for (var i = 0; i <= reply.RateReplyDetails.Length - 1; i++)
            {
                ...
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("Rate Reply Detail for Service " + (i + 1));
                }
                ...
                if (rateReplyDetail.ServiceTypeSpecified)
                {
                    if (DebugMode)
                    {
                        SepFunctions.Debug_Log("Service Type: " + rateReplyDetail.ServiceType);
                    }
                }
                ...
                if (rateReplyDetail.PackagingTypeSpecified)
                {
                    if (DebugMode)
                    {
                        SepFunctions.Debug_Log("Packaging Type: " + rateReplyDetail.PackagingType);
                    }
                }
                ...
                if (rateReplyDetail.RatedShipmentDetails != null)
                {
                    for (var j = 0; j <= rateReplyDetail.RatedShipmentDetails.Length - 1; j++)
                    {
                        ...
                        if (DebugMode)
                        {
                            SepFunctions.Debug_Log("---Rated Shipment Detail for Rate Type " + (j + 1) + "---");
                        }
        ...
        public void ShowShipmentRateDetails(ref RatedShipmentDetail shipmentDetail)
        {
            ...
            if (DebugMode)
            {
                SepFunctions.Debug_Log("--- Shipment Rate Detail ---");
                SepFunctions.Debug_Log("RateType: " + rateDetail.RateType);
            }
            ...
            if (rateDetail.TotalBillingWeight != null)
            {
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("Total Billing Weight: " + rateDetail.TotalBillingWeight.Value + " " + rateDetail.TotalBillingWeight.Units);
                }
            }
            ...
            if (rateDetail.TotalBaseCharge != null)
            {
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("Total Base Charge: " + rateDetail.TotalBaseCharge.Amount + " " + rateDetail.TotalBaseCharge.Currency);
                }
            }
            ...
            if (rateDetail.TotalFreightDiscounts != null)
            {
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("Total Freight Discounts: " + rateDetail.TotalFreightDiscounts.Amount + " " + rateDetail.TotalFreightDiscounts.Currency);
                }
            }
            ...
            if (rateDetail.TotalSurcharges != null)
            {
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("Total Surcharges: " + rateDetail.TotalSurcharges.Amount + " " + rateDetail.TotalSurcharges.Currency);
                }
            }
            ...
            if (rateDetail.Surcharges != null)
            {
                foreach (var surcharge in rateDetail.Surcharges)
                {
                    if (DebugMode)
                    {
                        SepFunctions.Debug_Log(" " + surcharge.SurchargeType + " surcharge " + surcharge.Amount.Amount + " " + surcharge.Amount.Currency);
                    }
                }
            }
            ...
            if (rateDetail.TotalNetCharge != null)
            {
                if (DebugMode)
                {
                    SepFunctions.Debug_Log("Total Net Charge: " + rateDetail.TotalNetCharge.Amount + " " + rateDetail.TotalNetCharge.Currency);
                }
