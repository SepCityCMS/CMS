<%@ page title="Zones" language="C#" viewstatemode="Enabled" masterpagefile="Site.Master"
    codebehind="zones.aspx.cs" inherits="wwwroot.zones" %>
...
<asp:content id="BodyContent" runat="server" contentplaceholderid="MainContent">
    ...
    <asp:Panel ID="UpdatePanel" runat="server">
        ...
        <h2>
            <span ID="PageHeader" runat="server" Text="Manage the Zones"></span>
        </h2>
						...
						<div class="input-group">
							<select id="FilterDoAction" runat="server" class="form-control" ClientIDMode="Static">
								<option value="">Select an Action</option>
									<option value="DeleteZones">Delete Zones</option>
                    </select>
							<span class="input-group-btn">
							<button class="btn btn-light" ID="RunAction" runat="server" OnServerClick="RunAction_Click" onclick="if(ExecuteAction(this, 'ZoneID') == false) {return false} else">Go!</button>
						</span>
					</div>
				</div>
				<div class="col-lg-6">
            ...
            <asp:GridView ID="ManageGridView" runat="server" AutoGenerateColumns="False" AllowSorting="True" ClientIDMode="Static"
                ...
                <Columns>
                    <asp:TemplateField ItemStyle-Width="20px">
                        ...
                        <ItemTemplate>
                            <input type="checkbox" id="ZoneID<%#
                Eval("ZoneID").ToString()%>" value="<%#
                Eval("ZoneID").ToString()%>" onclick="gridviewSelectRow(this);" />
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:templatefield ItemStyle-Width="20px">
                        <itemtemplate>
                            <a href="zones_modify.aspx?ZoneID=<%#
                Eval("ZoneID").ToString()%>&ModuleID=<%#
                SepCommon.SepFunctions.toLong(SepCommon.SepCore.Request.Item("ModuleID"))%>">
                                <img src="../images/public/edit.png" alt="Edit" />
                            </a>
                        </itemtemplate>
                    </asp:templatefield>
                    <asp:TemplateField HeaderText="Zone Name" HeaderStyle-HorizontalAlign="Left" ItemStyle-VerticalAlign="Top" SortExpression="ZoneName">
                        <ItemTemplate>
                            <%#
                Eval("ZoneName").ToString()%>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="Zone Template Code" HeaderStyle-HorizontalAlign="Left" ItemStyle-VerticalAlign="Top">
                        <ItemTemplate>
                            <%#
                                Convert.ToString(SepCommon.SepCore.Request.Item("ModuleID") == "1" ? "&lt;span class=&quot;contentrotator&quot;&gt;&lt;% var cContentRotator = new SepCityControls.ContentRotator();cContentRotator.ZoneID = &quot;" + Eval("ZoneID").ToString() + "&quot;;this.Response.Write(cContentRotator.Render()); %&gt;&lt;/span&gt;" : "")
                            %>
                            <%#
                                Convert.ToString(SepCommon.SepCore.Request.Item("ModuleID") == "2" ? "&lt;span class=&quot;adserver&quot;&gt;&lt;% var cAdServer = new SepCityControls.AdServer();cAdServer.ZoneID = &quot;" + Eval("ZoneID").ToString() + "&quot;;this.Response.Write(cAdServer.Render()); %&gt;&lt;/span&gt;" : "")
                            %>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
