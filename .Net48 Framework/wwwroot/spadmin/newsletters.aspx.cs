...
// ***********************************************************************
// <copyright file="newsletters.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class newsletters : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Newsletters");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Letter Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Description");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Sent");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Members");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No newsletters have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.Newsletters.Newsletter_Delete(sIDs);
        ...
        {
            var dNewsletters = SepCommon.DAL.Newsletters.GetNewsletters(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dNewsletters));
            return dv;
        }
