...
// ***********************************************************************
// <copyright file="portals.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class portals : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Portals");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Portal Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Description");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("View");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No portals have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.Portals.Portal_Delete(sIDs);
        ...
        {
            var dPortals = SepCommon.DAL.Portals.GetPortals(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value, -1);

            dv = new DataView(SepFunctions.ListToDataTable(dPortals));
            return dv;
        }
