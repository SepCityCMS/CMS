                    ...
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        if (RS.HasRows)
                        {
                            while (RS.Read())
                            {
                                ...
                                output.AppendLine("<option value=\"" + SepFunctions.openNull(RS["ActType"]) + "\"" + SepCommon.SepCore.Strings.ToString(SepFunctions.openNull(RS["ActType"]) == Text ? " selected=\"selected\"" : string.Empty) + ">" + SepFunctions.openNull(RS["ActType"]) + "</option>");
                            }
                        }
                    }
                }
