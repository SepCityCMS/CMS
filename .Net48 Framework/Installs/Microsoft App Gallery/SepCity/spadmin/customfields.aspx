<%@ page title="Custom Fields" language="C#" viewstatemode="Enabled" masterpagefile="Site.Master"
    codebehind="customfields.aspx.cs" inherits="wwwroot.customfields" %>
...
<asp:content id="BodyContent" runat="server" contentplaceholderid="MainContent">
    ...
    <asp:Panel ID="UpdatePanel" runat="server">
        ...
        <h2>
            <span ID="PageHeader" runat="server" Text="Manage the Custom Fields"></span>
        </h2>
        ...
        <div class="panel panel-default" id="PageManageGridView" runat="server">
						...
						<div class="input-group">
							<select id="FilterDoAction" runat="server" class="form-control" ClientIDMode="Static">
								<option value="">Select an Action</option>
									<option value="DeleteFields">Delete Fields</option>
                    </select>
							<span class="input-group-btn">
							<button class="btn btn-light" ID="RunAction" runat="server" OnServerClick="RunAction_Click" onclick="if(ExecuteAction(this, 'FieldID') == false) {return false} else">Go!</button>
						</span>
					</div>
				</div>
				<div class="col-lg-6">
            ...
            <input type="hidden" ID="ModuleID" runat="server" ClientIDMode="Static" Value="0" />
            ...
            <asp:GridView ID="ManageGridView" runat="server" AutoGenerateColumns="False" AllowSorting="True" ClientIDMode="Static"
                ...
                Eval("FieldID").ToString()%>" value="<%#
                Eval("FieldID").ToString()%>" onclick="gridviewSelectRow(this);" />
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:templatefield ItemStyle-Width="20px">
                        <itemtemplate>
                            <a href="customfields_modify.aspx?FieldID=<%#
                Eval("FieldID").ToString()%>&ModuleID=<%#
                SepCommon.SepFunctions.toLong(SepCommon.SepCore.Request.Item("ModuleID"))%>">
                                <img src="../images/public/edit.png" alt="Edit" />
                            </a>
                        </itemtemplate>
                    </asp:templatefield>
                    <asp:TemplateField HeaderText="Field Name" HeaderStyle-HorizontalAlign="Left" ItemStyle-VerticalAlign="Top" SortExpression="FieldName">
                        <ItemTemplate>
                            <%#
                Eval("FieldName").ToString()%>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="Answer Type" HeaderStyle-HorizontalAlign="Left" ItemStyle-VerticalAlign="Top" SortExpression="AnswerType">
                        <ItemTemplate>
                            <%#
                Format_Field(Eval("AnswerType").ToString())%>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="Required" HeaderStyle-HorizontalAlign="Left" ItemStyle-VerticalAlign="Top" SortExpression="Required">
                        <ItemTemplate>
                            <%#
                Eval("Required").ToString()%>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
