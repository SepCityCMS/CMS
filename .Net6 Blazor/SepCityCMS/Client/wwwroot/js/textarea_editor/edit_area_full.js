function EAL() {
    ...
    t.advanced_buttons = [
    ["new_document", "newdocument.gif", "new_document", false], ["search", "search.gif", "show_search", false],
    ["go_to_line", "go_to_line.gif", "go_to_line", false], ["undo", "undo.gif", "undo", true],
    ...
    ["highlight", "highlight.gif", "change_highlight", true], ["help", "help.gif", "show_help", false],
    ["save", "save.gif", "save", false], ["load", "load.gif", "load", false],
    ...
    if (t
        ...
        t.isOpera >= 9 ||
        t.isCamino) t.isValidBrowser = true;
...
EAL.prototype = {
    ...
    start: function(id) {
        ...
        if (!t.lang[eAs[id]["settings"]["language"]] ||
        (eAs[id]["settings"]["syntax"].length > 0 && !t.load_syntax[eAs[id]["settings"]["syntax"]])) {
            setTimeout("eAL.start('" + id + "');", 50);
            return;
        }
        ...
        (eAs[id]["settings"]["debug"] || eAs[id]["settings"]["allow_toggle"])) {
            ...
            if (eAs[id]["settings"]["debug"])
                html += "<textarea id='edit_area_debug_" +
                    id +
        ...
        for (var i = 0; i < area["settings"]["tab_toolbar"].length; i++) {
            html_toolbar_content += t
                .get_control_html(area["settings"]["tab_toolbar"][i], area["settings"]["language"]);
        }
        ...
        if (!t.iframe_script) {
            ...
            for (var i = 0;
                ...
                t.iframe_script += '<script language="javascript" type="text/javascript" src="' +
                    t.baseURL +
        ...
        if (typeof(window
            .frames["frame_" + area["settings"]["id"]]) !=
        "undefined") delete window.frames["frame_" + area["settings"]["id"]];
    ...
    get_control_html: function(button_name, lang) {
        ...
        for (var i = 0; i < t.advanced_buttons.length; i++) {
    ...
    setFileEditedMode: function(id, file_id, to) {
        ...
        reg1 = new RegExp("\\\\", "g");
        reg2 = new RegExp('"', "g");
        return this.execCommand(id,
            'set_file_edited_mode("' + file_id.replace(reg1, "\\\\").replace(reg2, '\\"') + '",' + to + ")");
    },
    execCommand: function(id, cmd, fct_param) {
        switch (cmd) {
        case "EA_init":
            if (eAs[id]["settings"]["EA_init_callback"]
                .length >
                0) eval(eAs[id]["settings"]["EA_init_callback"] + "('" + id + "');");
            break;
        case "EA_delete":
            if (eAs[id]["settings"]["EA_delete_callback"]
                .length >
                0) eval(eAs[id]["settings"]["EA_delete_callback"] + "('" + id + "');");
            break;
        case "EA_submit":
            if (eAs[id]["settings"]["submit_callback"]
                .length >
                0) eval(eAs[id]["settings"]["submit_callback"] + "('" + id + "');");
            break;
        }
    ...
    if (elem) {
        ...
        for (var x = 0; x < children.length; x++) {
            ...
            if (strTagName != "undefined") {
                ...
                if ((strTagName.toLowerCase() == elem_type.toLowerCase() || elem_type == "") &&
        ...
        if (!this.syntax[lang]) {
            ...
            if (this.load_syntax[lang]["KEYWORDS"]) {
                ...
                for (var i in this.load_syntax[lang]["KEYWORDS"]) {
                    ...
                    this
                        .syntax[lang]["keywords_reg_exp"][i] = new
                        RegExp(this.get_regexp(this.load_syntax[lang]["KEYWORDS"][i]), param);
            ...
            if (syntax_trace.length > 0)
                this
                    .syntax[lang]["syntax_trace_regexp"] = new
            ...
            if (this.load_syntax[lang]["REGEXPS"]) {
                for (var i in this.load_syntax[lang]["REGEXPS"]) {
                    ...
                    if (!this
                        .syntax[lang]["custom_regexp"][val["execute"]]
                    ) this.syntax[lang]["custom_regexp"][val["execute"]] = {};
                    this
                        .syntax[lang]["custom_regexp"][val["execute"]][i] =
                        { 'regexp': new RegExp(val["search"], val["modifiers"]), 'class': val["class"] };
                }
            }
