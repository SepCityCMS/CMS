...
// ***********************************************************************
// <copyright file="zones.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class zones : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Zones");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Zone Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Zone Template Code");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No zones have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.Zones.Zone_Delete(sIDs);
        ...
        {
            var dZones = SepCommon.DAL.Zones.GetZones(SepFunctions.toInt(SepCommon.SepCore.Request.Item("ModuleID")), GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dZones));
            return dv;
        }
