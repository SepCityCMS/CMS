<%@ page title="FAQ" language="C#" viewstatemode="Enabled" masterpagefile="Site.Master"
    codebehind="faq.aspx.cs" inherits="wwwroot.faq" %>
...
<asp:content id="BodyContent" runat="server" contentplaceholderid="MainContent">
    ...
    <asp:Panel ID="UpdatePanel" runat="server">
        ...
        <h2>
            <span ID="PageHeader" runat="server" Text="Manage the FAQ"></span>
        </h2>
						...
						<div class="input-group">
							<select id="FilterDoAction" runat="server" class="form-control" ClientIDMode="Static">
								<option value="">Select an Action</option>
									<option value="DeleteFAQs">Delete FAQ's</option>
                    </select>
							<span class="input-group-btn">
							<button class="btn btn-light" ID="RunAction" runat="server" OnServerClick="RunAction_Click" onclick="if(ExecuteAction(this, 'FAQID') == false) {return false} else">Go!</button>
						</span>
					</div>
				</div>
				<div class="col-lg-6">
            ...
            <asp:GridView ID="ManageGridView" runat="server" AutoGenerateColumns="False" AllowSorting="True" ClientIDMode="Static"
                ...
                <Columns>
                    <asp:TemplateField ItemStyle-Width="20px">
                        ...
                        <ItemTemplate>
                            <input type="checkbox" id="FAQID<%#
                Eval("FAQID").ToString()%>" value="<%#
                Eval("FAQID").ToString()%>" onclick="gridviewSelectRow(this);" />
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:templatefield ItemStyle-Width="20px">
                        <itemtemplate>
                            <a href="faq_modify.aspx?CatID=<%#
                Eval("CatID").ToString()%>&FAQID=<%#
                Eval("FAQID").ToString()%>">
