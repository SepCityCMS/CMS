...
// ***********************************************************************
// <copyright file="shoppingcart.aspx.cs" company="SepCity, Inc.">
    ...
    public partial class shoppingcart : Page
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
                {
                    ...
                    using (var cmd = new SqlCommand("SELECT ScriptText FROM Scripts WHERE ScriptType='GATEWAYS' AND PortalIDs LIKE '%|0|%'", conn))
                    {
        ...
        protected void SaveButton_Click(object sender, EventArgs e)
        {
            ...
            using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                using (var cmd = new SqlCommand("SELECT ID FROM Scripts WHERE ScriptType='GATEWAYS' AND PortalIDs LIKE '%|0|%'", conn))
                {
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                ...
                if (bUpdate)
                    SqlStr = "UPDATE Scripts SET ScriptText=@ScriptText WHERE ScriptType='GATEWAYS' AND PortalIDs LIKE '%|0|%'";
                else
                    SqlStr = "INSERT INTO Scripts (ScriptType, Description, ScriptText, PortalIDs, DatePosted) VALUES ('GATEWAYS', 'Payment Gateways', @ScriptText, '|0|', @DatePosted)";
