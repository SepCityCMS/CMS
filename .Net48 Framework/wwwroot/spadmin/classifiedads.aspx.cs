...
// ***********************************************************************
// <copyright file="classifiedads.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class classifiedads : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No classified ads have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    switch (FilterDoAction.Value)
                    {
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.Classifieds.Classified_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.Classifieds.Classified_Change_Status(sIDs, 0);
                            break;
                    }
                }
                catch
        ...
        {
            var dClassifiedAds = SepCommon.DAL.Classifieds.GetClassifiedAds(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dClassifiedAds));
            return dv;
        }
