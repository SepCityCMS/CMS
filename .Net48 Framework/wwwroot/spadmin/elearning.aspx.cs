...
namespace wwwroot
{
    ...
    public partial class elearning : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Courses");
                    ...
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Course Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Start Date");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("End Date");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Status");
                    ManageGridView.Columns[6].HeaderText = SepFunctions.LangText("Credits");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No courses have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    ...
                    {
                        case "DeleteCourses":
                            var sDeleteResult = SepCommon.DAL.ELearning.Course_Delete(sIDs);
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.ELearning.Course_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.ELearning.Course_Change_Status(sIDs, 0);
                            break;
                    }
                }
                catch
        ...
        {
            var dELearningCourses = SepCommon.DAL.ELearning.GetELearningCourses(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dELearningCourses));
            return dv;
        }
