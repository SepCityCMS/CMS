                    ...
                    using (SqlDataReader RS = cmd.ExecuteReader())
                    {
                        while (RS.Read())
                        {
                            if (string.IsNullOrWhiteSpace(SepFunctions.openNull(RS["AccessKeys"])) || SepFunctions.CompareKeys(SepFunctions.openNull(RS["AccessKeys"]), false))
                            {
                                ...
                                output.AppendLine("<li><img alt=\"" + aCounter + "\" src=\"" + screenShot + "\"" + SepCommon.Core.SepCore.Strings.ToString(SepFunctions.openNull(RS["TemplateID"]) == Text ? " class=\"selectedImage\"" : string.Empty) + " id=\"Template" + SepFunctions.openNull(RS["TemplateID"]) + "\" height=\"150\" width=\"190\" style=\"cursor:pointer;\" onclick=\"carouselClick('Template" + SepFunctions.openNull(RS["TemplateID"]) + "')\" /><span>" + SepFunctions.openNull(RS["TemplateName"]) + "</span></li>");
                            }
                        }
                    }
                }
