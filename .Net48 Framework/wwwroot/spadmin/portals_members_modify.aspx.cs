...
// ***********************************************************************
// <copyright file="portals_members_modify.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    /// <summary>
    /// Class portals_members_modify.
    /// Implements the <see cref="System.Web.UI.Page" />
    /// </summary>
    /// <seealso cref="System.Web.UI.Page" />
    public partial class portals_members_modify : Page
    {
        /// <summary>
            ...
            else
            {
                if (!Page.IsPostBack)
                    if (string.IsNullOrWhiteSpace(UserID.Value))
                        UserID.Value = SepFunctions.Generate_GUID();
            }
        }
        ...
        protected void SaveButton_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(Password.Value))
            {
                ...
                {
                    ErrorMessage.InnerHtml = "Password must contain one of @#$%^&*/!.";
                    return;
                }

                if (Regex.IsMatch(Password.Value, "[^\\s]{4,20}") == false)
                {
                    ErrorMessage.InnerHtml = "Password must be between 4-20 characters.";
                    return;
                }
            }

            var sReturn = SepCommon.DAL.Members.Member_Save(UserID.Value, UserName.Value, !string.IsNullOrWhiteSpace(Password.Value) ? Password.Value : string.Empty, string.Empty, string.Empty, FirstName.Value, LastName.Value, StreetAddress.Value, City.Value, State.Text, PostalCode.Value, Country.Text, EmailAddress.Value, PhoneNumber.Value, PayPalEmail.Value, SepFunctions.toLong(SepFunctions.GetUserInformation("UserPoints", UserID.Value)), SepFunctions.toLong(SepFunctions.GetUserInformation("AccessClass", UserID.Value)), SepFunctions.GetUserInformation("AccessKeys", UserID.Value), SepFunctions.toDate(BirthDate.Value), SepFunctions.toInt(Gender.Value), SepFunctions.GetUserInformation("AffiliateID", ReferralID.Value), WebsiteURL.Value, SepFunctions.toLong(SepFunctions.GetUserInformation("PortalID", UserID.Value)), Friends.Value, Request.Form["LetterIDs"], SepFunctions.toInt(Status.Value), string.Empty, string.Empty, string.Empty, false, SepFunctions.toLong(SepFunctions.GetUserInformation("SiteID", UserID.Value)), string.Empty);
