...
var dateFormat = function(date, numberFormatters, properties) {
    ...
    return properties.pattern.replace(datePatternRe,
        function(current) {
            ...
            case "Q":
            case "q":
                ret = Math.ceil((date.getDateAndTime.Month() + 1) / 3);
                if (length > 2) {
            ...
            case "M":
            case "L":
                ret = date.getDateAndTime.Month() + 1;
                if (length > 2) {
...
var dateIsLeapYear = function(year) {
    return new Date(year, 1, 29).getDateAndTime.Month() === 1;
};


/**
 * lastDayOfDateAndTime.Month( date )
 *
 * @date [Date]
 *
...
var dateLastDayOfMonth = function(date) {
    return new Date(date.getFullYear(), date.getDateAndTime.Month() + 1, 0).getDate();
};
...
var dateSetDate = function(date, day) {
    var lastDay = new Date(date.getFullYear(), date.getDateAndTime.Month() + 1, 0).getDate();
...
/**
 * Differently from native date.setDateAndTime.Month(), this function adjusts date if
 * needed, so final month is always the one set.
 *
 * setDateAndTime.Month( Jan31Date, 1 ): a "Feb 28" date.
...
var dateSetMonth = function(date, month) {
    ...
    date.setDate(1);
    date.setDateAndTime.Month(month);
...
var dateParse = function(value, tokens, properties) {
    ...
    valid = tokens.every(function(token) {
        ...
        case "L":
            ...
            dateSetDateAndTime.Month(date, value - 1);
    ...
    if (day !== undefined) {
        if (outOfRange(day, 1, dateLastDayOfDateAndTime.Month(date))) {
            return null;
        }
        date.setDate(day);
...
var dateTokenizer = function(value, numberParser, properties) {
    ...
    valid = properties.pattern.match(datePatternRe)
