jQuery.fn.colourPicker = function(conf) {
    ...
    var config = jQuery.extend({
            ...
            ico: "images/jquery.colourPicker.gif", // SRC to colour-picker icon
        ...
        jQuery(document.body)
            .click(function(event) {
                if (!(jQuery(event
                    .target)
                .is("#" + config.id) ||
                jQuery(event.target).parents("#" + config.id).length)) {
    ...
    return this.each(function() {
        ...
        var icon = jQuery('<a href="#"><img src="' + config.ico + '" alt="' + config.openTxt + '" /></a>')
            .insertAfter(select);
        var input = jQuery('<input type="text" name="' +
                select.attr("name") +
                '" class="ignore" value="' +
                select.val() +
                '" size="6" />')
