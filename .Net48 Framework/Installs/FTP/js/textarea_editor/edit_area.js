...
EditArea.prototype.init = function() {
    ...
    if (syntax_selec = _$("syntax_selection")) {
        ...
        for (var i = 0; i < t.syntax_list.length; i++) {
            ...
            option.innerHTML = typeof(dispSyntax) == "undefined"
    ...
    parent.editAreaLoader.add_event(t.result,
        "click",
        function(e) {
    ...
    parent.editAreaLoader.add_event(window,
        "unload",
        function() {
...
EditArea.prototype.update_size = function() {
    ...
    if (typeof editAreas != "undefined" && editAreas[editArea.id] && editAreas[editArea.id]["displayed"] == true) {
        if (editArea.fullscreen["isFull"]) {
            pd
                .getElementById("frame_" + editArea.id)
                .style.width = pd.getElementsByTagName("html")[0].clientWidth + "px";
            pd
                .getElementById("frame_" + editArea.id)
                .style.height = pd.getElementsByTagName("html")[0].clientHeight + "px";
        }
        ...
        if (editArea.tab_browsing_area.style.display == "block" && (!editArea.isIE || editArea.isIE >= 8)) {
            ...
            editArea.tab_browsing_area.style.height = (editArea.result.offsetTop -
...
EditArea.prototype.manage_size = function(onlyOneTime) {
    ...
    if (editAreas[this.id]["displayed"] == true && this.textareaFocused) {
        ...
        if (this.settings["word_wrap"]) {
            ...
            if (this.isSafari)
                newW -= 6;
            this.content_highlight.style.width = this.selection_field_text.style
                .width = this.selection_field.style.width = this.test_font_size.style.width = newW + "px";
        }
