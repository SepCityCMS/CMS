...
// ***********************************************************************
// <copyright file="eventcalendar.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class eventcalendar : Page
    {
        ...
        /// <param name="sDate">The s date.</param>
        /// <returns>System.String.</returns>
        public string Format_Time(string sDate)
        {
            return Strings.FormatDateTime(SepFunctions.toDate(sDate), Strings.DateNamedFormat.ShortTime);
        }
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Events");
                    ...
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Posted By");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Event Date");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Event Time");
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No events have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    ...
                    {
                        case "DeleteEvents":
                            var sDeleteResult = SepCommon.DAL.Events.Event_Delete(sIDs);
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.Events.Event_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.Events.Event_Change_Status(sIDs, 0);
                            break;
                    }
                }
                catch
        ...
        {
            var dEvents = SepCommon.DAL.Events.GetEvents(GridViewSortExpression, GetSortDirection(), searchWords: ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dEvents));
            return dv;
        }
