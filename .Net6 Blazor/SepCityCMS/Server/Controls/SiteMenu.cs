...
namespace SepCityCMS.Server.Controls
{
    ...
    public class SiteMenu
    {
        ...
        {
            get => SepCore.Strings.ToString(m_OverrideLinkClass);
        ...
        public string Render()
        {
            ...
            using (SqlConnection conn = new SqlConnection(SepFunctions.Database_Connection()))
            {
                ...
                if (SepFunctions.isUserPage())
                {
                    ...
                    {
                        using (SqlCommand cmd = new SqlCommand("SELECT PageID,PageName,PageTitle FROM UPagesPages WHERE MenuID=" + MenuID + " AND UserID='" + SepFunctions.GetUserID(SepCore.Request.Item("UserName")) + "' ORDER BY Weight,PageName", conn))
                        {
                ...
                else
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT MP.UniqueID,MP.PageID FROM PortalPages AS MP WHERE (MP.PortalIDs LIKE '%|" + SepFunctions.Get_Portal_ID() + "|%' OR MP.PortalIDs LIKE '%|-1|%') AND MP.MenuID=" + MenuID + " AND MP.Status=1", conn))
                    {
                        using (SqlDataReader RS = cmd.ExecuteReader())
                        {
                            while (RS.Read())
                            {
                                if (SepCore.Strings.InStr(sCheckDup, "|" + SepFunctions.openNull(RS["PageID"]) + "|") == 0)
                                {
                    ...
                    if (SepCore.Strings.Len(sId) > 0)
                    {
                        wc = " UniqueID IN (" + SepCore.Strings.Right(sId, SepCore.Strings.Len(sId) - 1) + ")";
                    }
        ...
        public string REP_Site_Menu_Loop(string ID, long PageID, long ModuleID, string UserPageName, string TargetWindow, string LinkText)
        {
            ...
            {
                case 201:
                    if (SepCore.Strings.Left(UserPageName, 2) == "~/" || (SepCore.Strings.Left(UserPageName, 1) != "/" && SepCore.Strings.Left(UserPageName, 4) != "www." && SepCore.Strings.Left(UserPageName, 5) != "http:" && SepCore.Strings.Left(UserPageName, 6) != "https:"))
                    {
                        GetPageName = sInstallFolder + SepCore.Strings.Replace(UserPageName, "~/", string.Empty);
                    }
                    else if (SepCore.Strings.Left(UserPageName, 4) == "www.")
                    {
            ...
            return SepCore.Strings.ToString(str);
        }
    }
}