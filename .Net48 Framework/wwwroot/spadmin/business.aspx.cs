...
// ***********************************************************************
// <copyright file="business.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class business : Page
    {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    switch (FilterDoAction.Value)
                    {
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.Businesses.Business_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.Businesses.Business_Change_Status(sIDs, 0);
                            break;
                    }
                }
                catch
        ...
        {
            var dBusinesses = SepCommon.DAL.Businesses.GetBusinesses(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);
