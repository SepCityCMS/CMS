...
// ***********************************************************************
// <copyright file="elearning_exams_modify.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    /// <summary>
    /// Class elearning_exams_modify.
    /// Implements the <see cref="System.Web.UI.Page" />
    /// </summary>
    /// <seealso cref="System.Web.UI.Page" />
    public partial class elearning_exams_modify : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Question");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Question");
                    RunAction.InnerText = SepFunctions.LangText("GO");
                    ModuleSearchButton.Text = SepFunctions.LangText("Search");
                    ModifyLegend.InnerHtml = SepFunctions.LangText("Add Exam");
                    ...
                    ExamNameLabel.InnerText = SepFunctions.LangText("Exam Name:");
                    ...
                    ExamNameRequired.ErrorMessage = SepFunctions.LangText("~~Exam Name~~ is required.");
        ...
        /// <summary>
        /// Handles the Click event of the AddQuestionButton control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs" /> instance containing the event data.</param>
        protected void AddQuestionButton_Click(object sender, EventArgs e)
        {
            SepFunctions.Redirect("elearning_questions_modify.aspx?ExamID=" + ExamID.Value + "&CourseID=" + SepCommon.SepCore.Request.Item("CourseID"));
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!Page.IsPostBack && !string.IsNullOrWhiteSpace(SepCommon.SepCore.Request.Item("ExamID")))
            {
                var jExams = SepCommon.DAL.ELearning.Exam_Get(SepFunctions.toLong(SepCommon.SepCore.Request.Item("ExamID")));
                ...
                {
                    ModifyLegend.InnerText = SepFunctions.LangText("Edit Exam");
                    ExamID.Value = SepCommon.SepCore.Request.Item("ExamID");
                    CourseID.Value = Strings.ToString(jExams.CourseID);
                    ExamName.Value = jExams.ExamName;
                    StartDate.Value = jExams.StartDate.ToShortDateString();
                    EndDate.Value = jExams.EndDate.ToShortDateString();
                    Description.Value = jExams.Description;
                    VideoFile.ContentID = SepCommon.SepCore.Request.Item("ExamID");
            ...
            {
                if (string.IsNullOrWhiteSpace(ExamID.Value)) ExamID.Value = Strings.ToString(SepFunctions.GetIdentity());
                VideoFile.ContentID = ExamID.Value;
                if (!Page.IsPostBack)
                {
                    CourseID.Value = SepCommon.SepCore.Request.Item("CourseID");
                    StartDate.Value = Strings.FormatDateTime(DateTime.Now, Strings.DateNamedFormat.GeneralDate);
                    EndDate.Value = Strings.FormatDateTime(DateAndTime.DateAdd(DateAndTime.DateInterval.Month, 1, DateTime.Now), Strings.DateNamedFormat.GeneralDate);
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.ELearning.Question_Delete(sIDs);
        ...
        {
            var intReturn = SepCommon.DAL.ELearning.Exam_Save(SepFunctions.toLong(ExamID.Value), SepFunctions.toLong(CourseID.Value), SepFunctions.Session_User_ID(), ExamName.Value, SepFunctions.toDate(StartDate.Value), SepFunctions.toDate(EndDate.Value), Description.Value);

            ErrorMessage.InnerHtml = Globals.GetSaveMessage(Master, GlobalVars.ModuleID, intReturn, string.Empty, "Exam");
        }
        ...
        {
            var dELearningExams = SepCommon.DAL.ELearning.GetELearningExamQuestions(SepFunctions.toLong(ExamID.Value), GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);
