...
namespace SepCommon.DAL
{
    ...
    public static class Businesses
    {
        ...
        public static string Business_Change_Status(string BusinessIDs, int Status)
        {
            ...
            return SepFunctions.LangText("Business(s) has been successfully deleted.");
        }
        ...
        public static List<Models.Businesses> GetBusinesses(string SortExpression = "DatePosted", string SortDirection = "DESC", string searchWords = "", string userId = "", long CategoryId = -1, string StartDate = "")
        {
            ...
            string wClause = "MOD.UserID=M.UserID AND Mod.LinkID=Mod.BusinessID AND Mod.Status <> -1";
            ...
            using (var ds = new DataSet())
            {
                using (var conn = new SqlConnection(SepFunctions.Database_Connection()))
                {
                    using (var cmd = new SqlCommand("SELECT Mod.BusinessID,Mod.CatID,Mod.BusinessName,Mod.Description,Mod.Visits,Mod.Status,Mod.DatePosted,Mod.UserID,M.UserName,(SELECT Count(CommentID) FROM Comments WHERE ModuleID='20' AND UniqueID=Mod.BusinessID) AS TotalComments,(SELECT TOP 1 Featured FROM PricingOptions WHERE ModuleID='20' AND UniqueID=Mod.LinkID) AS Featured,(SELECT TOP 1 BoldTitle FROM PricingOptions WHERE ModuleID='20' AND UniqueID=Mod.LinkID) AS BoldTitle,(SELECT TOP 1 Highlight FROM PricingOptions WHERE ModuleID='20' AND UniqueID=Mod.LinkID) AS Highlight FROM BusinessListings AS Mod,Members AS M" + SepFunctions.Category_SQL_Manage_Select(CategoryId, wClause) + " ORDER BY " + SortExpression + " " + SortDirection, conn))
                    {
                ...
                for (var i = 0; i <= ds.Tables[0].Rows.Count - 1; i++)
                {
                    ...
                    dBusinesses.UserID = SepFunctions.openNull(ds.Tables[0].Rows[i]["UserID"]);
                    dBusinesses.UserName = SepFunctions.openNull(ds.Tables[0].Rows[i]["UserName"]);
