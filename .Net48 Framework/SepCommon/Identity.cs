...
namespace SepCommon
{
    ...
    public static partial class SepFunctions
    {
        ...
        {
            string returnFunction = Strings.ToString(Math.Abs(GenerateCRC32(IdentityJet(word))));
            do
            {
        ...
        private static long AddCrc32(string Item, long crc32)
        {
            ...
            for (iCounter = 1; iCounter <= Strings.Len(Item); iCounter++)
            {
                ...
                int bCharValue = Strings.Asc(Strings.Mid(Item, iCounter, 1));
                long lAccValue = crc32 & 0xffffff00;
                lAccValue /= 0x100;
                lAccValue &= 0xffffff;
                long lIndex = crc32 & 0xff;
                lIndex ^= bCharValue;
                long lTableValue = CRC32Table[lIndex];
            ...
            if (static_InitCrc32_LastSeed != Seed)
            {
                ...
                for (iBytes = 0; iBytes <= 255; iBytes++)
                {
                    ...
                    for (var iBits = 0; iBits <= 7; iBits++)
                    {
                        ...
                        long lTempCrc32 = lCrc32 & 0xfffffffe;
                        lTempCrc32 /= 0x2;
                        lTempCrc32 &= 0x7fffffff;
