...
// ***********************************************************************
// <copyright file="advertising_stats.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    /// <summary>
    /// Class advertising_stats.
    /// Implements the <see cref="System.Web.UI.Page" />
    /// </summary>
    /// <seealso cref="System.Web.UI.Page" />
    public partial class advertising_stats : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                {
                    ManageGridView.Columns[0].HeaderText = SepFunctions.LangText("Site URL");
                    ManageGridView.Columns[1].HeaderText = SepFunctions.LangText("Start Date");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Total Exposures");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Total Clicks");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Max Clicks");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Ratio");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (SepFunctions.Setup(GlobalVars.ModuleID, "AdsStatsEnable") != "Yes" && Response.IsClientConnected)
                SepFunctions.Redirect(sInstallFolder + "error.aspx?ErrorID=404");
            SepFunctions.RequireLogin(SepFunctions.Security("AdsStats"));

            if (string.IsNullOrWhiteSpace(SepFunctions.Session_User_Name()) && Response.IsClientConnected)
            {
                SepCommon.SepCore.Session.setCookie("returnUrl", SepFunctions.GetInstallFolder() + "advertising_stats.aspx");
            ...
            {
                ErrorMessage.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No ad statistics are available.") + "</div>";
        ...
        {
            var dAdvertisementStats = SepCommon.DAL.Advertisements.GetAdvertisementStats(GridViewSortExpression, GetSortDirection(), UserID: SepFunctions.Session_User_ID());

            dv = new DataView(SepFunctions.ListToDataTable(dAdvertisementStats));
            return dv;
        }
