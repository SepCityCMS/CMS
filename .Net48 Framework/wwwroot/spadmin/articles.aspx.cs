...
namespace wwwroot
{
    ...
    public partial class articles : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Articles");
                    FilterDoAction.Items[2].Text = SepFunctions.LangText("Mark as Published");
                    FilterDoAction.Items[3].Text = SepFunctions.LangText("Mark as Pending");
                    FilterDoAction.Items[4].Text = SepFunctions.LangText("Mark as Archive");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Headline");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Author");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Status");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("Headline Date");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No articles have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    ...
                    {
                        case "DeleteArticles":
                            var sDeleteResult = SepCommon.DAL.Articles.Article_Delete(sIDs);
                        ...
                        case "MarkActive":
                            DeleteResult.InnerHtml = SepCommon.DAL.Articles.Article_Change_Status(sIDs, 1);
                            break;

                        case "MarkPending":
                            DeleteResult.InnerHtml = SepCommon.DAL.Articles.Article_Change_Status(sIDs, 0);
                            break;

                        case "MarkArchive":
                            DeleteResult.InnerHtml = SepCommon.DAL.Articles.Article_Change_Status(sIDs, 3);
                            break;
                    }
                }
                catch
        ...
        {
            var dArticles = SepCommon.DAL.Articles.GetArticles(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dArticles));
            return dv;
        }
