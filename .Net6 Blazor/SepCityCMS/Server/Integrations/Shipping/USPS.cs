...
namespace SepCityCMS.Server.Integrations.Shipping
{
    ...
    public class USPS : IDisposable
    {
        ...
        public void AddItem(int PackageNumber, string weight, string weightType, string shipperZipCode, string destinationZipCode, string serviceType)
        {
            ...
            sb.Append("<Package ID=\"" + Server.SepFunctions.NumberAddOrdinal(PackageNumber) + "\">");
        ...
        public decimal GetRate(string sXML)
        {
            if (DebugMode)
            {
                Server.SepFunctions.Debug_Log("USPS URL: " + m_uri + "&XML=" + Server.SepFunctions.UrlEncode(sXML));
            }

            var req = WebRequest.Create(m_uri + "&XML=" + Server.SepFunctions.UrlEncode(sXML));
            ...
            if (DebugMode)
            {
                Server.SepFunctions.Debug_Log("USPS Response: " + rawXml);
            }
            ...
            {
                rate = Server.SepFunctions.toDecimal(xml.SelectSingleNode("/RateV4Response/Package/Postage/Rate").InnerText);
            }
        ...
        public string GetShippingXML()
        {
            ...
            if (DebugMode)
            {
                Server.SepFunctions.Debug_Log(SepCore.Strings.ToString(sb));
            }
