...
// ***********************************************************************
// <copyright file="shopstores_modify.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    /// <summary>
    /// Class shopstores_modify.
    /// Implements the <see cref="System.Web.UI.Page" />
    /// </summary>
    /// <seealso cref="System.Web.UI.Page" />
    public partial class shopstores_modify : Page
    {
        /// <summary>
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                {
                    ModifyLegend.InnerHtml = SepFunctions.LangText("Edit Store");
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!Page.IsPostBack && !string.IsNullOrWhiteSpace(SepCommon.SepCore.Request.Item("StoreID")))
            {
                var jStores = SepCommon.DAL.ShoppingMall.Store_Get(SepFunctions.toLong(SepCommon.SepCore.Request.Item("StoreID")));

                if (jStores.StoreID == 0)
                {
                    ErrorMessage.InnerHtml = "<div class=\"alert alert-danger\" role=\"alert\">" + SepFunctions.LangText("~~Store~~ does not exist.") + "</div>";
                    ModFormDiv.Visible = false;
                }
                else
                {
                    ModifyLegend.InnerText = SepFunctions.LangText("Edit Store");
                    StoreID.Value = SepCommon.SepCore.Request.Item("StoreID");
                    UserID.Value = jStores.UserID;
                    StoreName.Value = jStores.StoreName;
                    Description.Value = jStores.Description;
                    CompanyName.Value = jStores.CompanyName;
                    StreetAddress.Value = jStores.StreetAddress;
                    City.Value = jStores.City;
                    Country.Text = jStores.Country;
                    State.Text = jStores.State;
                    PostalCode.Value = jStores.PostalCode;
                    PhoneNumber.Value = jStores.PhoneNumber;
                    FaxNumber.Value = jStores.FaxNumber;
                    SiteURL.Value = jStores.SiteURL;
                    ContactEmail.Value = jStores.ContactEmail;
                }
            }
            else
            {
