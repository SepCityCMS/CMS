    ...
    $.extend($.fn,
    {
        miniColors: function(o, data) {
            ...
            var create = function(input, o, data) {
                ...
                input.on("keydown.miniColors",
                    function(event) {
                        if (event.keyCode === 9) hide(input);
                    });

                // Update when color is typed in
                input.on("keyup.miniColors",
                    function(event) {
                        setColorFromInput(input);
                    });

                // Handle pasting
                input.on("paste.miniColors",
                    function(event) {
            ...
            var show = function(input) {
                ...
                var trigger = input.data("trigger"),
                    ...
                    top = hidden
                        ? trigger.offset().top + trigger.outerHeight()
                        : input.offset().top + input.outerHeight(),
                ...
                if (input.data("open")) {
                    input.data("open")
                        .call(input.get(0),
                            "#" + hsb2hex(hsb),
                            $.extend(hsb2rgb(hsb), { a: parseFloat(input.attr("data-opacity")) }));
                }
            ...
            var setColor = function(input, hsb, updateInput) {
                ...
                if (input.data("opacity")) {
                    input.data("trigger")
                        .css("backgroundColor",
                            "rgba(" + rgb.r + ", " + rgb.g + ", " + rgb.b + ", " + input.attr("data-opacity") + ")");
                }
