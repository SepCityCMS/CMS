...
// ***********************************************************************
// <copyright file="guestbook.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    public partial class guestbook : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Entries");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("User Name");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Message");
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No guestbook entries have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.Guestbook.Entry_Delete(sIDs);
        ...
        {
            var dGuestbookEntries = SepCommon.DAL.Guestbook.GetGuestbookEntries(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dGuestbookEntries));
            return dv;
        }
