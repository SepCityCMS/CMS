...
// ***********************************************************************
// <copyright file="messenger_blocked_users.aspx.cs" company="SepCity, Inc.">
...
namespace wwwroot
{
    ...
    /// <summary>
    /// Class messenger_blocked_users.
    /// Implements the <see cref="System.Web.UI.Page" />
    /// </summary>
    /// <seealso cref="System.Web.UI.Page" />
    public partial class messenger_blocked_users : Page
    {
        /// <summary>
        /// The c common
        /// </summary>
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Un-block User(s)");
        ...
        {
            var sReturn = SepCommon.DAL.Messenger.Message_Block_User_Save(SepFunctions.Session_User_ID(), Username.Value);

            DeleteResult.InnerHtml = sReturn;
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            {
                SepCommon.SepCore.Session.setCookie("returnUrl", SepFunctions.GetInstallFolder() + "messenger_blocked_users.aspx");
                SepFunctions.Redirect(sInstallFolder + "login.aspx");
            }

            var arrHeader = Strings.Split(SepFunctions.PageHeader(GlobalVars.ModuleID), "|$$|");
            Array.Resize(ref arrHeader, 3);
            Page.Title = arrHeader[0] + " - " + SepFunctions.LangText("Blocked Users");
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("You have not blocked any users.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.Messenger.Block_User_Delete(SepFunctions.Session_User_ID(), sIDs);
        ...
        {
            var dMessagesBlockedUsers = SepCommon.DAL.Messenger.GetMessagesBlockedUsers(ModuleSearch.Value, SepFunctions.Session_User_ID());

            dv = new DataView(SepFunctions.ListToDataTable(dMessagesBlockedUsers));
            return dv;
        }
    }
}