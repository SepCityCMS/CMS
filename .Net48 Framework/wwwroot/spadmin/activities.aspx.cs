...
namespace wwwroot
{
    ...
    public partial class activities : Page
    {
        ...
        public void TranslatePage()
        {
            if (!Page.IsPostBack)
            {
                ...
                if (SepFunctions.DebugMode || (sSiteLang != "EN-US" && !string.IsNullOrWhiteSpace(sSiteLang)))
                {
                    ...
                    FilterDoAction.Items[1].Text = SepFunctions.LangText("Delete Activities");
                    ManageGridView.Columns[2].HeaderText = SepFunctions.LangText("Date Posted");
                    ManageGridView.Columns[3].HeaderText = SepFunctions.LangText("Activity Type");
                    ManageGridView.Columns[4].HeaderText = SepFunctions.LangText("Description");
                    ManageGridView.Columns[5].HeaderText = SepFunctions.LangText("IP Address");
                }
            }
        }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ManageGridView.Rows.Count == 0)
            {
                DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No activities have been added.") + "</div>";
                PageManageGridView.Visible = false;
            }
        }
        ...
        protected void RunAction_Click(object sender, EventArgs e)
        {
            ...
            if (Strings.Len(sIDs) > 0)
            {
                try
                {
                    var sDeleteResult = SepCommon.DAL.Activities.ActivityDelete(sIDs);
        ...
        {
            var dActivities = SepCommon.DAL.Activities.GetActivities(GridViewSortExpression, GetSortDirection(), ModuleSearch.Value, SepCommon.SepCore.Request.Item("UserID"), SepFunctions.toInt(SepCommon.SepCore.Request.Item("ModuleID")));

            dv = new DataView(SepFunctions.ListToDataTable(dActivities));
            return dv;
        }
