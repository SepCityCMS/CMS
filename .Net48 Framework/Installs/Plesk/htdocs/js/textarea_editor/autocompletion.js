...
var EditArea_autocompletion = {
    ...
    onload: function() {
        ...
        parent.editAreaLoader.add_event(editArea.textarea,
            "blur",
            function() { editArea.plugins["autocompletion"]._hide(); });
    ...
    onkeydown: function(e) {
        ...
        setTimeout("editArea.plugins['autocompletion']._checkDelayAndCursorBeforeDisplay();",
    ...
    _checkDelayAndCursorBeforeDisplay: function() {
        this.checkDelayTimer = setTimeout("if(editArea.textarea.selectionStart == " +
    ...
    _select: function(content) {
        ...
        if (nbMatch > 0)
            parent.editAreaLoader.setSelectionRange(editArea.id,
            ...
            if (!parent.editAreaLoader.syntax[lang]["autocompletion"]) // init the regexp if not already initialized
            {
                ...
                if (parent.editAreaLoader.load_syntax[lang]["AUTO_COMPLETION"]) {
                    ...
                    for (var i in parent.editAreaLoader.load_syntax[lang]["AUTO_COMPLETION"]) {
                        ...
                        tmp["match_prefix_separator"] = new
                            RegExp(datas["REGEXP"]["prefix_separator"] + "$", tmp["modifiers"]);
                        tmp["match_word"] = new RegExp("(?:" +
                            ...
                            ")$",
                            tmp["modifiers"]);
                        tmp["match_next_letter"] = new
                            RegExp("^(" + datas["REGEXP"]["letter_after_word_must_match"] + ")$", tmp["modifiers"]);
                        tmp["keywords"] = {};
                        ...
                        for (var prefix in datas["KEYWORDS"]) {
                            ...
                            for (var j = 0; j < datas["KEYWORDS"][prefix].length; j++) {
                                ...
                                if (tmp["keywords"][prefix]["datas"][j]["replace_with"].length == 0)
                                    tmp["keywords"][prefix]["datas"][j]["replace_with"] = tmp["keywords"][prefix][
                                        "datas"][j]["is_typing"];

                                // if the comment is empty, display the replace_with value
                                if (tmp["keywords"][prefix]["datas"][j]["comment"].length == 0)
                                    tmp["keywords"][prefix]["datas"][j]["comment"] = tmp["keywords"][prefix]["datas"][j]
                                    ["replace_with"].replace(/{@}/g, "");
                            }
        ...
        if (editArea.is_editable) {
            ...
            for (var i in this.curr_syntax) {
                ...
                if (matchNextletter || this.forceDisplay) {
                    ...
                    if (match_word) {
                        ...
                        var match_curr_word = new RegExp("^" + parent.editAreaLoader.get_escaped_regexp(begin_word),
                        ...
                        for (var prefix in this.curr_syntax[i]["keywords"]) {
                            ...
                            for (var j = 0; j < this.curr_syntax[i]["keywords"][prefix]["datas"].length; j++) {
                                ...
                                if (this.curr_syntax[i]["keywords"][prefix]["datas"][j]["is_typing"]
                    ...
                    // we'll display to list of all available words
                        ...
                        for (var prefix in this.curr_syntax[i]["keywords"]) {
                            for (var j = 0; j < this.curr_syntax[i]["keywords"][prefix]["datas"].length; j++) {
                                ...
                                // we still need to check the prefix if there is one
                                else if (match_prefix_separator &&
                                    this.curr_syntax[i]["keywords"][prefix]["prefix"].length > 0) {
            ...
            if (results.length == 1 && this.autoSelectIfOneResult) {
                ...
                for (var i = 0; i < results.length; i++) {
                    var line = "<li><a href=\"#\" class=\"entry\" onmousedown=\"EditArea_autocompletion._select('" +
                        results[i][1]["replace_with"].replace(new RegExp('"', "g"), "&quot;") +
                        "');return false;\">" +
                        results[i][1]["comment"];
