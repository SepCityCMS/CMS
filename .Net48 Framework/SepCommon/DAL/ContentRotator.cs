...
namespace SepCommon.DAL
{
    ...
    public static class ContentRotator
    {
        ...
        public static string Content_Delete(string ContentIDs)
        {
            ...
            return SepFunctions.LangText("Content has been successfully deleted.");
        }
        ...
        public static int Content_Save(long ContentID, string UserID, string Description, long ZoneID, string HTMLCode, string CatIDs, string PageIDs, string PortalIDs)
        {
            ...
            int intReturn = SepFunctions.Additional_Data_Save(isNewRecord, 1, Strings.ToString(ContentID), UserID, SepFunctions.GetUserInformation("UserName", UserID), "Content", "Content", string.Empty);
            if (intReturn == 0)
            {
                if (bUpdate)
                {
