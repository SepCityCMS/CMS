...
EditArea.prototype.maj_highlight = function(infos) {
    ...
    var textToHighlight = infos["full_text"],
        doSyntaxOpti = false,
        doHtmlOpti = false,
        stay_begin = "",
        stay_end = "",
    ...
    if (doSyntaxOpti) {
        try {
            ...
            replacedBloc = t.last_hightlighted_text
                .substring(stay_begin.length, t.last_hightlighted_text.length - stay_end.length);

            lengthOld = replacedBloc.length;
            lengthNew = updated_highlight.length;

            // find the identical caracters at the beginning
            for (var i = 0; i < lengthOld && i < lengthNew && replacedBloc.charAt(i) == updated_highlight.charAt(i); i++) {
            }
            nbStart = i;
            ...
            for (var i = 0;
                i + nbStart < lengthOld &&
                    i + nbStart < lengthNew &&
                    replacedBloc.charAt(lengthOld - i - 1) == updated_highlight.charAt(lengthNew - i - 1);
                i++) {
            }
            nbEnd = i;
            ...
            if (newHtml.indexOf("<span") == -1 &&
                ...
                // get the last opened span
                    ...
                    nbClosed = beginStr.substr(Math.max(0, beginStr.lastIndexOf("<span", maxStartOffset - 1)))
                        .split("</span")
                        .length -
                        1;
                    while (nbClosed > 0) {
                    ...
                    while (parentSpan.parentNode != t.content_highlight &&
                        ...
                        (tmpMaxStartOffset = Math.max(0, beginStr.lastIndexOf("<span", maxStartOffset - 1))) < (tmpMaxEndOffset = Math.max(0, beginStr.lastIndexOf("</span", maxEndOffset - 1)))) {
                ...
                if ((lastIndex = beginStr.lastIndexOf(">")) == -1) {
                    ...
                    nbUnchangedChars = beginStr.substr(lastIndex + 1)
                        .replace(/&lt;/g, "<")
                        .replace(/&gt;/g, ">")
                        .replace(/&amp;/g, "&")
                        .length;
                ...
                if (t.isIE) {
                    ...
                    textNode.replaceData(nbUnchangedChars,
                        lastHtml.replace(/\n/g, "").length,
                        newHtml.replace(/\n/g, ""));
                } else {
    ...
    if (!doHtmlOpti) {
        ...
        if ((t.isIE && t.isIE < 8) || (t.isOpera && t.isOpera < 9.6))
            new_Obj
                .innerHTML = "<pre><span class='" + t.settings["syntax"] + "'>" + hightlighted_text + "</span></pre>";
        else
    ...
    if (t.settings["debug"]) {
        ...
        t.debug.value = "Tps optimisation " +
        (tps_end_opti - tps_start) +
        " | tps reg exp: " +
        (tpsAfterReg - tps_end_opti) +
        " | tps opti HTML : " +
        (tpsAfterOpti2 - tpsAfterReg) +
        " " +
        (doHtmlOpti ? "yes" : "no") +
        " | tps update highlight content: " +
        (tps3 - tpsAfterOpti2) +
        " | tpsTotal: " +
        (tps3 - tps_start) +
        "(" +
