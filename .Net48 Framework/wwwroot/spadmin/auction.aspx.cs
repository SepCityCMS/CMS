    ...
    public partial class auction : Page
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (ManageGridView.Rows.Count == 0)
                {
                    DeleteResult.InnerHtml = "<div class=\"alert alert-info\" role=\"alert\">" + SepFunctions.LangText("No auction ads have been added.") + "</div>";
                    PageManageGridView.Visible = false;
                }
            }
        }
        ...
        {
            var dAuctionAds = SepCommon.DAL.Auctions.GetAuctionAds(searchWords: ModuleSearch.Value);

            dv = new DataView(SepFunctions.ListToDataTable(dAuctionAds));
            return dv;
        }
    }
}